...
Partial Class ISOS2ISIP
    ...
    Sub SetDefaultValue()
        ...
        AtSales.Style("left") = -500 & "px"
        AtDevlop.Style("left") = -500 & "px"
        AtFAS.Style("left") = -500 & "px"
        '
        BSPInq.Style("left") = -500 & "px"
        LSearch.Style("left") = -500 & "px"
        LTeethPage.Style("left") = -500 & "px"
        LTapeDesc.Style("left") = -500 & "px"
        '
    End Sub
    ...
    Sub ShowData()
        ...
        If pBuyer = "000055" Then
            ...
            If AtBuyer.Checked = True Then
                ...
                '
                sql = sql + "'兼' as SLDC, "
                sql = sql + "'http://10.245.1.6/WorkFlowSub/ISOSGetColorFA300.aspx?pUserID=" & UserID & "&pBuyer=" & pBuyer & "&pColor=' + LTRIM(RTRIM(C1)) "
                sql = sql + " AS SLDCURL, "
                '
                sql &= "A1, B1, C1, D1, E1, "
                sql &= "F1, G1, H1, I1, J1, "
            ...
            If AtBuyer.Checked = True Then
                ...
                If DColorType.SelectedValue = "TAPE" Then
                    ...
                    If (e.Row.RowType = DataControlRowType.Header) Then
                        ...
                        tcl(0).Text = "A1" & "<BR>" & "Type"
                        tcl.Add(New TableHeaderCell())
                        tcl(1).Text = "B1" & "<BR>" & "Color Specification"
                        tcl.Add(New TableHeaderCell())
                        tcl(2).Text = "C1" & "<BR>" & "Stander Tape"
                        tcl(2).BackColor = Color.Green
                        tcl.Add(New TableHeaderCell())
                        tcl(3).Text = "D1" & "<BR>" & "CNT10"
                        tcl.Add(New TableHeaderCell())
                        tcl(4).Text = "E1" & "<BR>" & "CNT8"
                        tcl.Add(New TableHeaderCell())
                        tcl(5).Text = "F1" & "<BR>" & "VT8"
                        tcl.Add(New TableHeaderCell())
                        tcl(6).Text = "G1" & "<BR>" & "VT10"
                        tcl.Add(New TableHeaderCell())
                        tcl(7).Text = "H1" & "<BR>" & "Remark"
                    End If
                    '
