...
Partial Class ISOS2FAS_AFItemColor_01
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "ISOS2FAS_AFItemColor_01.aspx"  '程式名
        '-----------------------------------------------------------------
        '-- 共用參數
        '-----------------------------------------------------------------
    ...
    Sub ShowItemDataList()
        ...
        ' 篩選資料
        sql = "SELECT "
        sql &= "a.CustCode As CustCode, b.CustName + '(' + a.CustCode + ')' As Customer, a.Buyer, a.Season, a.Month, a.CustItem, a.KeyData2, "
        ...
        sql &= MMSQLStr
        sql &= "      ) "
        sql &= "Group by a.CustCode, b.CustName, a.Buyer, a.Season, a.Month, a.CustItem, a.KeyData2 "
        sql &= "Order by a.CustCode, b.CustName, a.Buyer, a.Season, a.Month, a.CustItem, a.KeyData2 "
        '
    ...
    Protected Sub ITEMGridView_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles ITEMGridView.RowDataBound
        ...
        If (e.Row.RowType = DataControlRowType.DataRow) Then
            ...
            Dim URLACT As String = "http://10.245.0.205/EDI/InfF_FCTACTVDPAnalysis_03.aspx?" + _
                                   "pBuyer=" + gBuyer + _
            ...
            Dim URLFCT As String = "http://10.245.0.205/EDI/InfF_FCTACTVDPAnalysis_03.aspx?" + _
                                   "pBuyer=" + gBuyer + _
                                   "&pCustCode=" + DataBinder.Eval(e.Row.DataItem, "CustCode") + _
                                   "&pSeason=" + gSeason + _
                                   "&pMonth=" + gMonth + _
            ...
            If dt_FCTQty.Rows.Count > 0 Then
                ...
                Hyper1.NavigateUrl = URLACT + "&pOption=ACT"
                Hyper1.Target = "_blank"
                e.Row.Cells(5).Controls.Add(Hyper1)
                '
                Hyper2.Text = Format(dt_FCTQty.Rows(0).Item("FCTQty"), "###,###,###")
                Hyper2.NavigateUrl = URLFCT + "&pOption=FCT"
                Hyper2.Target = "_blank"
                e.Row.Cells(6).Controls.Add(Hyper2)
                '
                If dt_FCTQty.Rows(0).Item("FCTQty") > 0 Then
                    e.Row.Cells(7).Text = Format(dt_FCTQty.Rows(0).Item("ACTQty") / dt_FCTQty.Rows(0).Item("FCTQty") * 100, ".0") + "%"
                    '
