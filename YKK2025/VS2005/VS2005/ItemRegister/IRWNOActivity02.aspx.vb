...
Partial Class IRWNOActivity02
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "IRWNOActivity02.aspx"     '程式名
        '-----------------------------------------------------------------
        '-- 共用參數
        '-----------------------------------------------------------------
        NowDateTime = Now.ToString("yyyy/MM/dd HH:mm:ss")   '現在日期時間
        UserID = Request.QueryString("pUserID")             'UserID
        pEmpID = Request.QueryString("pEmpID")             'UserID
    ...
    Sub SetDefaultValue()
        ...
        For i As Integer = 0 To dt.Rows.Count - 1
            ...
            ListItem2.Text = dt.Rows(i).Item("YYMM")
            ListItem2.Value = dt.Rows(i).Item("YYMM")
            DYYMM.Items.Add(ListItem2)
        Next
        '
        If pYYMM <> "" Then DYYMM.SelectedIndex = 3
        '
        If pSTS <> "" Then DStatus.SelectedIndex = 1
    End Sub
    '---------------------------------------------------------------------------------------------------
    '*****************************************************************
    '**(BGo)
    ...
    Sub ShowData()
        ...
        '篩選資料
        Sql = "SELECT "
        Sql = Sql & "case when wno='' then 'NG' else 'YES' end as Status,  "

        Sql = Sql & "convert(varchar, Date, 111) AS xDate, "
        Sql = Sql & "convert(varchar, CompletedTime, 111) AS xCompletedTime, "

        Sql = Sql & "WNo, No, DepName, Name, ForUse,  "
        Sql = Sql & "Code, ItemName1, ItemName2, ItemName3, Customer, Buyer, "
        Sql = Sql & "FormSno, CCode, "

        Sql &= "'http://10.245.1.6/IRW/ItemRegisterSheet_02.aspx?pFormNo=001151&pFormSno=' + rtrim(ltrim(str(formsno))) as URL "

        Sql = Sql & "From W_IRWTDF00 "
        Sql = Sql & "Where EmpID = '" & pEmpID & "' "

        If DYYMM.Text <> "ALL" Then Sql &= "And YYMM = '" & DYYMM.Text & "' "
        If DStatus.Text = "NG" Then Sql &= "And wNo = '" & "" & "' "
        If DStatus.Text = "YES" Then Sql &= "And wNo <> '" & "" & "' "
...
LBL_End:
    ...
    Protected Sub GridView1_RowDataBound1(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        If (e.Row.RowType = DataControlRowType.Header) Then
            ...
            tcl(7).Text = "CODE"
            tcl.Add(New TableHeaderCell())
            tcl(8).Text = "ITEM NAME 1"
            tcl.Add(New TableHeaderCell())
            tcl(9).Text = "ITEM NAME 2"
            tcl.Add(New TableHeaderCell())
            tcl(10).Text = "ITEM NAME 3"
            tcl.Add(New TableHeaderCell())
            tcl(11).Text = "客戶"
            tcl.Add(New TableHeaderCell())
            tcl(12).Text = "BUYER"
    ...
    Protected Sub BExcel_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles BExcel.Click
        ...
        Response.AppendHeader("Content-Disposition", "attachment;filename=IRWNOActivityList02.xls")     '程式別不同
