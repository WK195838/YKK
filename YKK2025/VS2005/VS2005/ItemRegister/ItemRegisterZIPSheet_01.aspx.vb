...
Partial Class ItemRegisterZIPSheet_01
    ...
    Dim wApplyID As String          '申請者IDapplytype
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ' JOY-NOTICE PAGE
            Response.Write("<script>window.open('http://10.245.1.6/IRW/IRWNoticePageZip.aspx','','height=800,width=800,menubar=no,location=no');</script>")
    ...
    Sub TopPosition()
        ...
        DAttachfile1.Style("top") = Top + 57 & "px"
    ...
    Sub GetData()
        ...
        Else
            SQL = "Select FormSno From M_Form "
            SQL &= "Where FormNo  = '" & wFormNo & "' "
            SQL &= "  And Active = '1' "
            Dim dt_FormSeq As DataTable = uDataBase.GetDataTable(SQL)
            wNo = SetNo(dt_FormSeq.Rows(0).Item("FormSno"))
        End If
        SQL = "Select "
        SQL = SQL & "'ItemRegisterSheet_02.aspx?pFormNo=001151&pFormSno=' + LTrim(RTrim(str(FormSno))) + '&pStep=' + '" + CStr(wStep) + "' + '&pUserID=' + '" + Request.QueryString("pUserID") + "' "
        SQL = SQL & "As FNoURL, "
        ...
        '
        SQL = SQL & "Case (Select Top 1 F_ItemRegisterSheetDetail.ApplyType From F_ItemRegisterSheetDetail Where F_ItemRegisterSheetDetail.FNo=V_ItemRegisterSheet_02.No And F_ItemRegisterSheetDetail.DataType=0 Order by F_ItemRegisterSheetDetail.ApplyType) "
        SQL = SQL & "When 0 Then 'Y' "
        SQL = SQL & "When 1 Then 'Y' "
        SQL = SQL & "Else ' ' End As ZIP, "
        '
        SQL = SQL & "Case (Select Top 1 F_ItemRegisterSheetDetail.ApplyType From F_ItemRegisterSheetDetail Where F_ItemRegisterSheetDetail.FNo=V_ItemRegisterSheet_02.No And F_ItemRegisterSheetDetail.DataType=1 Order by F_ItemRegisterSheetDetail.ApplyType) "
        SQL = SQL & "When 0 Then 'Y' "
        SQL = SQL & "When 1 Then 'Y' "
        SQL = SQL & "Else ' ' End As SLD, "
        '
        SQL = SQL & "Case (Select Top 1 F_ItemRegisterSheetDetail.ApplyType From F_ItemRegisterSheetDetail Where F_ItemRegisterSheetDetail.FNo=V_ItemRegisterSheet_02.No And F_ItemRegisterSheetDetail.DataType=2 Order by F_ItemRegisterSheetDetail.ApplyType) "
        SQL = SQL & "When 0 Then 'Y' "
        SQL = SQL & "When 1 Then 'Y' "
        SQL = SQL & "Else ' ' End As CH, "
        '
    ...
    Sub SetParameter()
        ...
        '
        Response.Cookies("PGM").Value = "ItemRegisterZIPSheet_01.aspx"
    ...
    Sub SetFieldAttribute(ByVal pPost As String)
        ...
        Select Case FindFieldInf("Attachfile1")
            ...
            Case 1  '修改+檢查
                ShowRequiredFieldValidator("DAttachfileRqd", "DAttachfile1", "異常：需輸入附件")
        ...
        End Select
        If Not IsPostBack Then
            If pPost = "New" Then LAttachfile1.Visible = False
        End If
    End Sub
    '*****************************************************************
    '**(SetFieldData)
    ...
    Sub AppendData(ByVal pFun As String, ByVal NewFormSno As Integer)
        Dim Path As String = Server.MapPath(uCommon.GetAppSetting("ItemRegisterZIPFilePath"))
        ...
        uDataBase.ExecuteNonQuery(sql)
        '
        'Update F_ItemRegisterSheetDetail Stats = 1
        sql = "Update F_ItemRegisterSheetDetail Set "
        sql &= " Sts = '1', "
    ...
    Sub ModifyData(ByVal pFun As String, ByVal pSts As String)
        ...
        sql = "Update F_ItemRegisterZIPSheet Set "
        If pFun <> "SAVE" Then
            sql &= " Sts = '" & pSts & "',"
        ...
        If DAttachfile1.Visible Then
            If DAttachfile1.PostedFile.FileName <> "" Then  '判斷有檔案上傳
                '*** IE8對應-Start 2011/1/4
                'Dim FileName As String = CStr(wFormSno) & "-" & "Attachfile1" & "-" & UploadDateTime & "-" & Right(DAttachfile1.PostedFile.FileName, InStr(StrReverse(DAttachfile1.PostedFile.FileName), "\") - 1)
                Dim FileName As String = CStr(wFormSno) & "-" & "Attachfile1" & "-" & UploadDateTime & "-" & IO.Path.GetFileName(DAttachfile1.PostedFile.FileName)
                '*** IE8對應-End
                Try    '上傳圖檔
                    DAttachfile1.PostedFile.SaveAs(Path + FileName)
                Catch ex As Exception
                End Try
                sql = sql + " AttachFile1 = '" + FileName + "', "
            End If
        End If
        '
        sql &= " ModifyUser = '" & Request.QueryString("pUserID") & "',"
    ...
    Function InputDataOK(ByVal pAction As Integer) As Boolean
        ...
        If ErrCode = 0 Then
            If DAttachfile1.Visible Then
                ...
                Else
                    If pAction = 0 Then ErrCode = 9010
                End If
            End If
        End If
        ...
        If ErrCode = 0 Then
            If DNo.Text <> "" Then
                ErrCode = oCommon.CommissionNo("001152", wFormSno, wStep, DNo.Text) '表單號碼, 表單流水號, 工程, 委託書No
                If ErrCode <> 0 Then
                    ErrCode = 9050
                End If
            End If
