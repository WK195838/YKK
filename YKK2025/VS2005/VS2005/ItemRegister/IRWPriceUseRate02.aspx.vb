...
Partial Class IRWPriceUseRate02
    ...
    Sub SetParameter()
        ...
        pDepCode = Request.QueryString("pDepCode")             'UserID
        '-----------------------------------------------------------------
        '-- 初值
        '-----------------------------------------------------------------
    ...
    Sub ShowData()
        ...
        '篩選資料
        Sql = "SELECT "
        Sql = Sql & "DepName, EmpName, MM1_YES, MM1_NO, MM2_YES, MM2_NO, MM3_YES, MM3_NO, "
        ...
        Sql = Sql & "EmpID "
        Sql = Sql & "From W_ValuationUseRate "
        Sql = Sql & "Where DataType = 'P' "
        Sql = Sql & "And DepCode = '" & pDepCode & "' "

        Sql = Sql & "Order By Total_Yes desc "
        Dim dt As DataTable = uDataBase.GetDataTable(Sql)
...
LBL_End:
    ...
    Protected Sub GridView1_RowDataBound1(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        If (e.Row.RowType = DataControlRowType.DataRow) Then
            ...
            '                If CDbl(e.Row.Cells(i).Text) >= 40 Then
                ...
                Select Case i
                    Case 4, 7, 10, 13, 16, 19, 22
                        If CDbl(e.Row.Cells(i - 2).Text) + CDbl(e.Row.Cells(i - 1).Text) > 0 Then
                            str = CDbl(e.Row.Cells(i - 2).Text) / ((CDbl(e.Row.Cells(i - 1).Text))) * 10000
            ...
            For i = 0 To 21
                Select Case i
                    Case 3, 6, 9, 12, 15, 18, 21
                        If e.Row.DataItemIndex > 0 Then e.Row.Cells(i).Text = ""
                    Case Else
                End Select
            Next
            '
