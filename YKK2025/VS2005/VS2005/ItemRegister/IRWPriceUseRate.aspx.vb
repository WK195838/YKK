...
Partial Class IRWPriceUseRate
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "IRWPriceUseRate.aspx"     '程式名
        '-----------------------------------------------------------------
        '-- 共用參數
        '-----------------------------------------------------------------
    ...
    Sub ShowData()
        ...
        '篩選資料
        Sql = "SELECT "
        Sql = Sql & "'.LINK.' AS DepName, MM1_YES, MM1_NO, MM2_YES, MM2_NO, MM3_YES, MM3_NO, "
        ...
        Sql = Sql & "DepCode, "
        '
        Sql &= "'IRWPriceUseRate01.aspx?pUserID=" & UserID & "&pDepCode=' + DepCode as URL "
        '
        Sql = Sql & "From W_ValuationUseRate "
        Sql = Sql & "Where DataType = 'C' "
        Sql = Sql & "Order By Total_No desc "
        Dim dt As DataTable = uDataBase.GetDataTable(Sql)
...
LBL_End:
                ...
                Select Case i
                    Case 3, 6, 9, 12, 15, 18, 21
                        If CDbl(e.Row.Cells(i - 2).Text) + CDbl(e.Row.Cells(i - 1).Text) > 0 Then
                            str = CDbl(e.Row.Cells(i - 2).Text) / ((CDbl(e.Row.Cells(i - 2).Text) + CDbl(e.Row.Cells(i - 1).Text))) * 10000
    ...
    Protected Sub BExcel_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles BExcel.Click
        ...
        Response.AppendHeader("Content-Disposition", "attachment;filename=IRWPriceUseRateList.xls")     '程式別不同
