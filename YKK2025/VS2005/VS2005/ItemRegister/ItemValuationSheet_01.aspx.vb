...
Partial Class ItemValuationSheet_01
    ...
    Dim PRICEConnect As String = uCommon.GetAppSetting("TP_Conn")
    ...
    Sub SetParameter()
        ...
        DItemValuationFile.Text = "\\10.245.0.192\program$\ISOS\DataPrepare\" + "ItemValuation_" & wApplyID & ".xlsm"
        '
        DNo.Style("left") = -500 & "px"
        DDate.Style("left") = -500 & "px"
        DName.Style("left") = -500 & "px"
        DJobTitle.Style("left") = -500 & "px"
        DDivision.Style("left") = -500 & "px"
        '
    End Sub
    '---------------------------------------------------------------------------------------------------
    '*****************************************************************
    ...
    Sub SetPopupFunction()
        ...
        DA001.Attributes.Add("onclick", "ValuationPriceVersion('A001')")    ' A001-PriceVersion
        DA206.Attributes.Add("onclick", "ValuationPriceVersion('A206')")    ' A206-PriceVersion
        DA211.Attributes.Add("onclick", "ValuationPriceVersion('A211')")    ' A211-PriceVersion
        DA999.Attributes.Add("onclick", "ValuationPriceVersion('A999')")    ' A999-PriceVersion
        DK206.Attributes.Add("onclick", "ValuationPriceVersion('K206')")    ' K206-PriceVersion
        DK211.Attributes.Add("onclick", "ValuationPriceVersion('K211')")    ' K211-PriceVersion
    ...
    Protected Sub BCheckItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BCheckItem.Click
        ...
        If Not InputError Then
            ...
            '        DStatus.Value = "NG"
            '        uJavaScript.PopMsg(Me, "[1101]:此Item資料已經申請,請確認 ! ")
            '    Else
            '        DStatus.Value = "OK"
            '    End If
            'Else
            ...
            '        DStatus.Value = "NG"
            '        uJavaScript.PopMsg(Me, "[1102]:此Item資料已經存在(Waves),請確認 ! ")
            '    Else
            '        DStatus.Value = "OK"
            '    End If
            'End If
            ...
            '估價不需-end
            '--------------------------------
            '[估價]CHECK SLIDER, FINISH
            Dim SQL As String
            Dim xPuller1, xPuller2, xSLDKey1, xSLDKey2 As String
            Dim cn As New OleDbConnection
            Dim ds As New DataSet
            cn.ConnectionString = PRICEConnect
            '
            If DStatus.Value = "OK" Then
                'PL-, T-PL-, B-PL-
                xPuller1 = ""
                xPuller2 = ""
                For i As Integer = 0 To xSRequest.Length - 1
                    If Mid(xSRequest(i), 1, 3) = "PL-" Or Mid(xSRequest(i), 1, 5) = "T-PL-" Or Mid(xSRequest(i), 1, 5) = "B-PL-" Then
                        If xPuller1 = "" Then
                            xPuller1 = xSRequest(i)
                        Else
                            xPuller2 = xSRequest(i)
                        End If
                    End If
                Next
                '
                xSLDKey1 = DSlider1.Text
                If Mid(xSLDKey1, 1, 4) = "DA7B" Or Mid(xSLDKey1, 1, 5) = "DAG7B" Then
                    If Mid(xPuller1, 1, 2) = "PL" Or Mid(xPuller1, 1, 2) = "T-" Then xSLDKey1 = xSLDKey1 & xPuller1
                    If Mid(xPuller2, 1, 2) = "PL" Or Mid(xPuller2, 1, 2) = "T-" Then xSLDKey1 = xSLDKey1 & xPuller2
                End If
                xSLDKey2 = DSlider2.Text
                If Mid(xSLDKey2, 1, 4) = "DA7B" Or Mid(xSLDKey2, 1, 5) = "DAG7B" Then
                    If Mid(xPuller1, 1, 2) = "PL" Or Mid(xPuller1, 1, 2) = "B-" Then xSLDKey2 = xSLDKey2 & xPuller1
                    If Mid(xPuller2, 1, 2) = "PL" Or Mid(xPuller2, 1, 2) = "B-" Then xSLDKey2 = xSLDKey2 & xPuller2
                End If
                '
                'Slider-1
                ds.Clear()
                If xSLDKey1 <> "" Then
                    If Mid(xSLDKey1, 1, 1) = "*" Then
                        '多拉頭
                        SQL = "SELECT Slider1 From M_ManySliderStructure "
                        SQL = SQL & "Where Active = '1' "
                        SQL = SQL & "And Size = '" & Trim(DSize.Text) & "' "
                        SQL = SQL & "And Family = '" & Trim(DFamily.Text) & "' "
                        SQL = SQL & "And Special = '" & Trim(xSLDKey1) & "' "
                        Dim DBAdapter1 As New OleDbDataAdapter(SQL, cn)
                        DBAdapter1.Fill(ds, "Slider")
                        If ds.Tables("Slider").Rows.Count <= 0 Then DStatus.Value = "W-SLD1/"
                    Else
                        '單拉頭
                        SQL = "SELECT Price From M_SliderPrice "
                        SQL = SQL & "Where Active = '1' "
                        If DA001.Checked = True Then
                            SQL = SQL & "And CType = 'A' "
                            SQL = SQL & "And Version = '001' "
                        End If
                        If DA999.Checked = True Then
                            SQL = SQL & "And CType = 'A' "
                            SQL = SQL & "And Version = '999' "
                        End If
                        If DA211.Checked = True Then
                            SQL = SQL & "And CType = 'A' "
                            SQL = SQL & "And Version = '211' "
                        End If
                        ...
                        SQL = SQL & "And Size = '" & Trim(DSize.Text) & "' "
                        SQL = SQL & "And Family = '" & Trim(DFamily.Text) & "' "
                        SQL = SQL & "And Slider = '" & Trim(xSLDKey1) & "' "
                        Dim DBAdapter1 As New OleDbDataAdapter(SQL, cn)
                        DBAdapter1.Fill(ds, "Slider")
                        If ds.Tables("Slider").Rows.Count <= 0 Then DStatus.Value = "W-SLD1/"
                    End If
                ...
                ds.Clear()
                If xSLDKey2 <> "" Then
                    If Mid(xSLDKey2, 1, 1) = "*" Then
                        '多拉頭
                        SQL = "SELECT Slider1 From M_ManySliderStructure "
                        SQL = SQL & "Where Active = '1' "
                        SQL = SQL & "And Size = '" & Trim(DSize.Text) & "' "
                        SQL = SQL & "And Family = '" & Trim(DFamily.Text) & "' "
                        SQL = SQL & "And Special = '" & Trim(xSLDKey2) & "' "
                        Dim DBAdapter2 As New OleDbDataAdapter(SQL, cn)
                        DBAdapter2.Fill(ds, "Slider")
                        If ds.Tables("Slider").Rows.Count <= 0 Then
                            If DStatus.Value = "OK" Then
                                DStatus.Value = "W-SLD2/"
                            Else
                                DStatus.Value = DStatus.Value & "SLD2/"
                            End If
                        End If
                    Else
                        '單拉頭
                        ...
                        SQL = SQL & "And Size = '" & Trim(DSize.Text) & "' "
                        SQL = SQL & "And Family = '" & Trim(DFamily.Text) & "' "
                        SQL = SQL & "And Slider = '" & Trim(xSLDKey2) & "' "
                        Dim DBAdapter2 As New OleDbDataAdapter(SQL, cn)
                        DBAdapter2.Fill(ds, "Slider")
                        If ds.Tables("Slider").Rows.Count <= 0 Then
                            If DStatus.Value = "OK" Then
                ...
                If DFinish1.Text <> "" Then
                    SQL = "SELECT Price From M_FinishAdd "
                    SQL = SQL & "Where Active = '1' "
                    If DA001.Checked = True Then
                        SQL = SQL & "And CType = 'A' "
                        SQL = SQL & "And Version = '001' "
                    ...
                    End If
                    SQL = SQL & "And Size = '" & Trim(DSize.Text) & "' "
                    SQL = SQL & "And Finish = '" & Trim(DFinish1.Text) & "' "
                    Dim DBAdapter3 As New OleDbDataAdapter(SQL, cn)
                    DBAdapter3.Fill(ds, "Finish")
                    If ds.Tables("Finish").Rows.Count <= 0 Then
                        If DStatus.Value = "OK" Then
                            DStatus.Value = "W-FIN1/"
                        Else
                            DStatus.Value = DStatus.Value & "FIN1/"
                        End If
                    End If
                End If
                '
                ...
                If DFinish2.Text <> "" Then
                    ...
                    End If
                    SQL = SQL & "And Size = '" & Trim(DSize.Text) & "' "
                    SQL = SQL & "And Finish = '" & Trim(DFinish2.Text) & "' "
                    Dim DBAdapter4 As New OleDbDataAdapter(SQL, cn)
                    DBAdapter4.Fill(ds, "Finish")
                    If ds.Tables("Finish").Rows.Count <= 0 Then
                        If DStatus.Value = "OK" Then
                            DStatus.Value = "W-FIN2/"
                        Else
                            DStatus.Value = DStatus.Value & "FIN2/"
                        End If
                    End If
                End If
            End If
            '
            If InStr(DStatus.Value, "W-") > 0 Then
                If InStr(DStatus.Value, "SLD") > 0 And InStr(DStatus.Value, "FIN") > 0 Then
                    uJavaScript.PopMsg(Me, "此Slider & Finish不存在(Price MST),請注意 ! ")
                Else
                    If InStr(DStatus.Value, "SLD") > 0 Then
                        uJavaScript.PopMsg(Me, "此Slider不存在(Price MST),請注意 ! ")
                    Else
                        uJavaScript.PopMsg(Me, "此Finish不存在(Price MST),請注意 ! ")
                    End If
                End If
            End If
            '--------------------------------
            '
            If DStatus.Value = "OK" Or InStr(DStatus.Value, "W-") > 0 Then
                ...
                '
                '[估價]資料存入
                SQL = "UPDATE F_ItemValuationSheet SET Sts = 1, "
                SQL = SQL & "RMDSIM = '" & "未估價" & "', "
    ...
    Public Function DataError() As Boolean
        ...
        If Not uError Then
            If DCustomerCode.Text = "" Or DBuyerCode.Text = "" Then
                uError = True
                If uError Then uJavaScript.PopMsg(Me, "[CUST-BUYER]:需輸入客戶及BUYER ?,請確認 ! ")
            End If
        End If
        '
        Return uError
    ...
    '*****************************************************************
    Sub AppendData()
        'Dim UploadDateTime As String = CStr(DateTime.Now.Year) + CStr(DateTime.Now.Month) + CStr(DateTime.Now.Day) + _
        ...
        Dim sql As String = ""
        sql = "INSERT INTO F_ItemValuationSheet ( " & _
