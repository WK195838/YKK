...
Partial Class ItemRegisterFSLDSheet_01
    ...
    Sub SetParameter()
        ...
        '
        Response.Cookies("PGM").Value = "ItemRegisterFSLDSheet_01.aspx"
    ...
    Sub ShowSheetFunction()
        ...
        GridView2.Style("top") = Top + 32 + 20 & "px"
    End Sub
    '---------------------------------------------------------------------------------------------------
    '*****************************************************************
    '**(ShowSheetField)
    ...
    Sub SetFieldAttribute(ByVal pPost As String)
        ...
        Select Case FindFieldInf("TYPE")
            ...
            Case 1  '修改+檢查
                ...
                ShowRequiredFieldValidator("DTYPERqd", "DTYPE", "異常：需輸入拉鍊區隔")
                DTYPE.Visible = True
            Case 2  '修改
            ...
            Case Else   '隱藏
                DTYPE.Visible = False
        End Select
        If Not IsPostBack Then SetFieldData("TYPE", "ZZZZZZ")
    ...
    Sub SetFieldData(ByVal pFieldName As String, ByVal pName As String)
        ...
        If pFieldName = "TYPE" Then
            ...
            If idx = 0 Then
                ...
                End If
            Else
                sql = "Select * From M_Referp Where Cat='1151' and DKey='TYPE' Order by DKey, Data "
                Dim dtTYPE As DataTable = uDataBase.GetDataTable(sql)
                For i As Integer = 0 To dtTYPE.Rows.Count - 1

                    Dim ListItem1 As New ListItem
                    ListItem1.Text = dtTYPE.Rows(i)("Data")
                    ListItem1.Value = dtTYPE.Rows(i)("Data")
                    DTYPE.Items.Add(ListItem1)
                Next
            End If
        End If
    ...
    '*****************************************************************
    Sub ShowFormData()
        Dim Path As String = uCommon.GetAppSetting("Http") & uCommon.GetAppSetting("ItemRegisterFSLDFilePath")
        Dim SQL As String
        SQL = "Select * From F_ItemRegisterFSLDSheet "
        ...
        If dtItemRegisterSheet.Rows.Count > 0 Then
            ...
            End If
            SetFieldData("TYPE", dtItemRegisterSheet.Rows(0).Item("TYPE"))
            '交易資料
            SQL = "Select * From T_WaitHandle "
            SQL = SQL & " Where Active = 1 "
            SQL = SQL & "   And FormNo =  '" & wFormNo & "'"
    ...
    Sub AppendData(ByVal pFun As String, ByVal NewFormSno As Integer)
        Dim Path As String = Server.MapPath(uCommon.GetAppSetting("ItemRegisterFSLDFilePath"))
        ...
        Dim sql As String = ""
        sql = "INSERT INTO F_ItemRegisterFSLDSheet ( " & _
              "Sts, CompletedTime, FormNo, FormSno, " & _
              "No, Date, Name, JobTitle, Division, " & _
              "Remark, Attachfile1,TYPE, " & _
    ...
    Sub ModifyData(ByVal pFun As String, ByVal pSts As String)
        ...
        sql = "Update F_ItemRegisterFSLDSheet Set "
        If pFun <> "SAVE" Then
            sql &= " Sts = '" & pSts & "',"
    ...
    Function InputDataOK(ByVal pAction As Integer) As Boolean
        ...
        If ErrCode = 0 Then
            If DNo.Text <> "" Then
                ErrCode = oCommon.CommissionNo("001155", wFormSno, wStep, DNo.Text) '表單號碼, 表單流水號, 工程, 委託書No
                If ErrCode <> 0 Then
                    ErrCode = 9050
                End If
            End If
