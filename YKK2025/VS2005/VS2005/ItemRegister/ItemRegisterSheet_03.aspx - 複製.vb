...
Partial Class ItemRegisterSheet_03
    ...
    Sub ShowSheetField(ByVal pPost As String)
        ...
        '違反者採用流程2開始
        If wStep = 1 Then
            If Request.QueryString("pUserID") = "it004" Or Request.QueryString("pUserID") = "???" Or Request.QueryString("pUserID") = "???" Then
                wStep = 3
            End If
        End If
        'ADD-END
    ...
    Public Function EDXNotFound(ByVal pCat As String) As Boolean
        ...
        msg = ""
        If Not uError Then
            'A.決定SLIDER CODE (縮寫對應) 
            '-------------------------------------
            If InStr(xSpc, "PL-") <= 0 Then
                '1.NK525 /N525/N52/
                If iNew(4) <> "" Then
                    sql = "SELECT Top 1 Puller, Short "
                    sql = sql & "From M_PullerShort "
                    sql = sql & "Where '" & iNew(4) & "' Like '%' + Short + '%' "
                    sql = sql & "Order By len(Short) desc, short Desc "
                    Dim dtPuller1 As DataTable = uDataBase.GetDataTable(sql)
                    If dtPuller1.Rows.Count > 0 Then
                        iNew(4) = Replace(iNew(4), dtPuller1.Rows(0)("Short"), dtPuller1.Rows(0)("Puller"))
                    End If
                End If
                If iNew(6) <> "" Then
                    sql = "SELECT Top 1 Puller, Short "
                    sql = sql & "From M_PullerShort "
                    sql = sql & "Where '" & iNew(6) & "' Like '%' + Short + '%' "
                    sql = sql & "Order By len(Short) desc, short "
                    Dim dtPuller2 As DataTable = uDataBase.GetDataTable(sql)
                    If dtPuller2.Rows.Count > 0 Then
                        iNew(6) = Replace(iNew(6), dtPuller2.Rows(0)("Short"), dtPuller2.Rows(0)("Puller"))
                    End If
                End If
            Else
                '2.特別要求PL- 05 CN DA7BA C5 PARTS KENSIN N-ANTI PL-Y749
                If iNew(4) <> "" Then
                    If Mid(DSRequest1.Text, 1, 3) = "PL-" Then iNew(4) = iNew(4) & DSRequest1.Text
                    If Mid(DSRequest2.Text, 1, 3) = "PL-" Then iNew(4) = iNew(4) & DSRequest2.Text
                    If Mid(DSRequest3.Text, 1, 3) = "PL-" Then iNew(4) = iNew(4) & DSRequest3.Text
                    If Mid(DSRequest4.Text, 1, 3) = "PL-" Then iNew(4) = iNew(4) & DSRequest4.Text
                    If Mid(DSRequest5.Text, 1, 3) = "PL-" Then iNew(4) = iNew(4) & DSRequest5.Text
                    If Mid(DSRequest6.Text, 1, 3) = "PL-" Then iNew(4) = iNew(4) & DSRequest6.Text
                End If
                If iNew(6) <> "" Then
                    If Mid(DSRequest1.Text, 1, 3) = "PL-" Then iNew(6) = iNew(6) & DSRequest1.Text
                    If Mid(DSRequest2.Text, 1, 3) = "PL-" Then iNew(6) = iNew(6) & DSRequest2.Text
                    If Mid(DSRequest3.Text, 1, 3) = "PL-" Then iNew(6) = iNew(6) & DSRequest3.Text
                    If Mid(DSRequest4.Text, 1, 3) = "PL-" Then iNew(6) = iNew(6) & DSRequest4.Text
                    If Mid(DSRequest5.Text, 1, 3) = "PL-" Then iNew(6) = iNew(6) & DSRequest5.Text
                    If Mid(DSRequest6.Text, 1, 3) = "PL-" Then iNew(6) = iNew(6) & DSRequest6.Text
                End If
            End If
            '
            'B.3Y SALES & FA000 (M_WINGSEDX)
            MsgBox(iNew(4))
            If iNew(4) <> "" Then
                uRun = False
                sql = "SELECT Top 1 Slider, OrderNo, Item "
                sql = sql & "From M_WINGSEDX "
                sql = sql & "Where Slider = '" & iNew(4) & "' "
                sql = sql & "Order By OrderNo desc, Item desc "
                Dim dtSales1 As DataTable = uDataBase.GetDataTable(sql)
                If dtSales1.Rows.Count > 0 Then
                    DSPDNOUrl.Text = DSPDNOUrl.Text & Chr(13) & "SLD-1:EDX[" & dtSales1.Rows(0)("OrderNo") & "/" & dtSales1.Rows(0)("Item") & "]"
                Else
                    DSPDNOUrl.Text = DSPDNOUrl.Text & Chr(13) & "SLD-1:EDX[NOT FOUND:3Y SALES & WINGS]"
                    uRun = True
                End If
                '
                'C.EDX DB (V_EDX) (SPD, PRICE, EDX WFS, HAND, 5Y_QC)
                If uRun Then
                    '3Y SALES = 無
                    sql = "SELECT Top 1 Size, Family, Body, Puller, Color, Finish "
                    sql = sql & "From V_EDX "
                    sql = sql & "Where Puller + Color = '" & Mid(iNew(4), 2, 999) & "' "
                    sql = sql & "   Or Puller + Color = '" & Mid(iNew(4), 3, 999) & "' "
                    sql = sql & "   Or Puller + Color = '" & Mid(iNew(4), 4, 999) & "' "
                    sql = sql & "   Or Puller + Color = '" & Mid(iNew(4), 5, 999) & "' "
                    sql = sql & "Order By len(Puller) desc, Color Desc "
                    Dim dtEDXDB1 As DataTable = uDataBase.GetDataTable(sql)
                    If dtEDXDB1.Rows.Count > 0 Then
                        DSPDNOUrl.Text = DSPDNOUrl.Text & Chr(13) & "SLD-1:EDX[" & dtEDXDB1.Rows(0)("PULLER") & dtEDXDB1.Rows(0)("COLOR") & "]"
                    Else
                        DSPDNOUrl.Text = DSPDNOUrl.Text & Chr(13) & "SLD-1:EDX[NOT FOUND:EDX DB]"
                        msg = iNew(4) & ":" & "搜尋不到[EDX]，請確認是否已完成 ?"
                        uError = True
                    End If
                End If
            End If
            '
            If Not uError And iNew(6) <> "" Then
                uRun = False
                sql = "SELECT Top 1 Slider, OrderNo, Item "
                sql = sql & "From M_WINGSEDX "
                sql = sql & "Where Slider = '" & iNew(6) & "' "
                sql = sql & "Order By OrderNo desc, Item desc "
                Dim dtSales2 As DataTable = uDataBase.GetDataTable(sql)
                If dtSales2.Rows.Count > 0 Then
                    DSPDNOUrl.Text = DSPDNOUrl.Text & Chr(13) & "SLD-2:EDX[" & dtSales2.Rows(0)("OrderNo") & "/" & dtSales2.Rows(0)("Item") & "]"
                Else
                    DSPDNOUrl.Text = DSPDNOUrl.Text & Chr(13) & "SLD-2:EDX[NOT FOUND:3Y SALES & WINGS]"
                    uRun = True
                End If
                '
                'C.EDX DB (V_EDX) (SPD, PRICE, EDX WFS, HAND, 5Y_QC)
                If uRun Then
                    ...
                    sql = "SELECT Top 1 Size, Family, Body, Puller, Color, Finish "
                    sql = sql & "From V_EDX "
                    sql = sql & "Where Puller + Color = '" & Mid(iNew(6), 2, 999) & "' "
                    sql = sql & "   Or Puller + Color = '" & Mid(iNew(6), 3, 999) & "' "
                    sql = sql & "   Or Puller + Color = '" & Mid(iNew(6), 4, 999) & "' "
                    sql = sql & "   Or Puller + Color = '" & Mid(iNew(6), 5, 999) & "' "
                    sql = sql & "Order By len(Puller) desc, Color Desc "
                    Dim dtEDXDB2 As DataTable = uDataBase.GetDataTable(sql)
                    If dtEDXDB2.Rows.Count > 0 Then
                        DSPDNOUrl.Text = DSPDNOUrl.Text & Chr(13) & "SLD-2:EDX[" & "/" & dtEDXDB2.Rows(0)("PULLER") & dtEDXDB2.Rows(0)("COLOR") & "]"
                    Else
                        DSPDNOUrl.Text = DSPDNOUrl.Text & Chr(13) & "SLD-2:EDX[NOT FOUND:EDX DB]"
                        msg = iNew(4) & ":" & "搜尋不到[EDX]，請確認是否已完成 ?"
                        uError = True
                    End If
                End If
    ...
    Public Function NotEDXItem(ByVal pSlider As String, ByVal pBuyer As String) As Boolean
        ...
        If pSlider <> "" Then
            ...
            If Mid(pSlider, 1, 4) = "DA7B" Or Mid(pSlider, 1, 5) = "DAG7B" Then
                'PULLER編成
    ...
    Function InputDataOK(ByVal pAction As Integer) As Boolean
        ...
        If ErrCode = 0 Then
            ...
            If (wStep <= 3 Or wStep = 500 Or wStep = 503) And pAction = 0 Then
                ...
                If ErrCode = 0 Then
                    '生產NG-檢測 (BUYER)(JOY)
                    If ItemNotProd("BUYER") Then
                        ErrCode = 9900
                    Else
                        'MODIFY-START ISOS-2308 PJ
                        'EDX CHECK (OLD)
                        'If NotEDXItem(DSlider1.Text, DBuyerCode.Text) Then
                        '    ErrCode = 9910
                        'Else
                        '    If NotEDXItem(DSlider2.Text, DBuyerCode.Text) Then
                        '        ErrCode = 9920
                        '    Else
                        '        '記得拿()
                        '        ErrCode = 9999
