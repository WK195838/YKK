...
Partial Class MPMProcessesSheet_02
    ...
    Sub ShowFormData()
        ...
        SQL = SQL & " Where FormNo =  '" & CStr(wFormNo) & "'"
        ...
        If DBAdapter1.Rows.Count > 0 Then
            ...
            DDivision.Text = DBAdapter1.Rows(0).Item("Division")
        ...
        SQL = SQL & "  and  delmark =0 and FormSNo =  '" & CStr(wFormSno) & "' )  And delmark = 0 And FormSNo  ='" & CStr(wFormSno) & "'  ) as LEnddate  "
        SQL = SQL & " From F_MPMProcessesSheetDT a "
        SQL = SQL & " Where delmark =0 and recdate is not null and FormSNo =  '" & CStr(wFormSno) & "' )a order by SeqNo"
        ...
        SQL = SQL & " ( (convert(char(8),startdate,108)<'11:50:00'  and  convert(char(8),Enddate,108)>'12:35:00' ) "
        SQL = SQL & "  or (day(startdate) <> day(enddate) and convert(char(8),StartDate,108)<'11:50:00' ) or "
        SQL = SQL & " (day(startdate) <> day(enddate) and convert(char(8),Enddate,108)>'12:35:00' ))"
        SQL = SQL & " then 0 else 0 end as distime, worktime  from F_MPMProcessesSheetdt "
        SQL = SQL & " Where delmark =0 and recdate is not null and FormSNo =  '" & CStr(wFormSno) & "' "
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        If Enginecode = "線割(機)" Or Enginecode = "放電(機)" Or Enginecode = "CNC(機)" Or Enginecode = "CAM(先行)" Or Enginecode = "CAM(先行)" Or Enginecode = "CNC電極" Then
    ...
    'Function GetURL(ByVal UrlStr As String) As String
    '    '判斷檔案是否存在 
    '    If File.Exists(UrlStr) Then
    '        Path = uCommon.GetAppSetting("Http") & uCommon.GetAppSetting("MPMProcessesFilePath")
    '        '  Dim Path As String = ConfigurationSettings.AppSettings.Get("Http") & ConfigurationSettings.AppSettings.Get("ManufOutFilePath")
    '    Else
    '        Path = uCommon.GetAppSetting("HttpOld") & uCommon.GetAppSetting("MPMProcessesFilePath")
    '    End If
    'End Function
