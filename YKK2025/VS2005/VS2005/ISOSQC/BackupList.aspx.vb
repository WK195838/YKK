...
Partial Class BackupList
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "BackupList.aspx"   '程式名
        '-----------------------------------------------------------------
        '-- 共用參數
        '-----------------------------------------------------------------
    ...
    Sub SetDefaultValue()
        ...
        Dim i As Integer
        '
        DBackupVersion.Items.Clear()
        SQL = "SELECT HSCODE "
        SQL = SQL & "FROM B_PullerColor "
        SQL = SQL & "WHERE HSCODE in "
        SQL = SQL & " (select DISTINCT TOP 6 HSCODE from B_PullerColor order by HSCODE DESC)  "
        SQL = SQL & "group by HSCODE "
        SQL = SQL & "order by HSCODE desc "
        Dim dt_BackupVersion As DataTable = uDataBase.GetDataTable(SQL)
        For i = 0 To dt_BackupVersion.Rows.Count - 1
            Dim ListItem1 As New ListItem
            ListItem1.Text = dt_BackupVersion.Rows(i).Item("HSCODE")
            ListItem1.Value = dt_BackupVersion.Rows(i).Item("HSCODE")
            If i = 0 Then
                ListItem1.Selected = True
            Else
                ListItem1.Selected = False
            End If
            DBackupVersion.Items.Add(ListItem1)
        Next
    End Sub
    '---------------------------------------------------------------------------------------------------
    '*****************************************************************
    ...
    Sub ShowData()
        ...
        SQL = SQL & "FROM B_PullerColor "
        '
        SQL = SQL & "where HSCODE = '" & DBackupVersion.SelectedValue & "' "
        ' Puller
