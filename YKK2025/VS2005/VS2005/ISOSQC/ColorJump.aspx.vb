...
Partial Class ColorJump
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "ColorJump.aspx"   '程式名
        '-----------------------------------------------------------------
        '-- 共用參數
        '-----------------------------------------------------------------
    ...
    Sub ShowData()
        ...
        '
        SQL = "SELECT TOP 150 "
        SQL = SQL & "case when active='1' then 'OPEN' else 'CLOSE' end as Sts, "
        SQL = SQL & "case when active='1' then '使用中' else '停止使用' end as StsDesc, "
        SQL = SQL & "[UserName],[Userid],[DepName],[Puller],[Yobi1],[Yobi2], convert(varchar, CreateTime, 111) as CreateTime "
        SQL = SQL & "FROM M_NewPullerUser "
        '
        SQL = SQL & "where active <> 9 "
        ' Puller
        If DKPullerCode.Text <> "" Then
            SQL = SQL & "and puller LIKE '%" & DKPullerCode.Text & "%' "
        End If
        ' Other
        If DKOther.Text <> "" Then
            SQL = SQL & "and [UserName]+[Userid]+[DepName]+[Puller]+[Yobi1]+[Yobi2] like '%" & DKOther.Text & "%' "
        End If
        ' AtOPEN
        ...
        End If
        '
        SQL = SQL & "Order by puller, len(puller) desc "
        '
    ...
    Protected Sub GridView1_RowCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowCreated
        ...
        If (e.Row.RowType = DataControlRowType.Header) Then
            ...
            tcl.Add(New TableHeaderCell())
            tcl(2).Text = "UserName" & "<BR>" & ""
            tcl(2).BackColor = Color.Blue
            tcl.Add(New TableHeaderCell())
            tcl(3).Text = "Userid" & "<BR>" & ""
            tcl(3).BackColor = Color.Blue
            tcl.Add(New TableHeaderCell())
            tcl(4).Text = "DepName" & "<BR>" & ""
            tcl(4).BackColor = Color.Green
            tcl.Add(New TableHeaderCell())
            tcl(5).Text = "Puller" & "<BR>" & ""
