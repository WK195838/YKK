...
Partial Class WIPList
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "Drop.aspx"   '程式名
        '-----------------------------------------------------------------
        '-- 共用參數
        '-----------------------------------------------------------------
    ...
    Sub ShowData()
        ...
        '
        SQL = SQL & "where [RD]+[DTM]+[EDX]+[IRW]+[ORDERS] LIKE '%WIP%' "
        SQL = SQL & "AND CREATEUSER not in ('SP_DRR') "
        'SQL = SQL & "AND CREATEUSER not in ('SP_DRR','SP_Combine') "
        ' Puller
        ...
        End If
        ' AtRD
        If AtRD.Checked = True Then
            SQL = SQL & "and RD = 'WIP' "
        End If
        ' AtDTM
        If AtDTM.Checked = True Then
            SQL = SQL & "and DTM = 'WIP' "
        End If
        ' AtEDX
        If AtEDX.Checked = True Then
            SQL = SQL & "and EDX = 'WIP' "
        End If
        ' AtIRW
