...
Partial Class UNDERARMOURBYFlow
    ...
    Sub SetParameter()
        ...
        '動作按鈕設定
        BBYFMTChange.Attributes("onclick") = "var ok=window.confirm('" + "是否開啟Excel準備資料？" + "');if(!ok){return false;} else {CheckAttribute('NIKEBYImport')}"
    ...
    Protected Sub BBYFMTChange_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles BBYFMTChange.Click
        ...
        Else
            ...
            uJavaScript.PopMsg(Me, "準備 BY FCT 資料未完成，請確認!")
        End If
        '
    End Sub
    '---------------------------------------------------------------------------------------------------
