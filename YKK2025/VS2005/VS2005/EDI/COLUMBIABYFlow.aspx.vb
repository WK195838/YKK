...
Partial Class COLUMBIABYFlow
    ...
    Sub SetParameter()
        ...
        '動作按鈕設定
        BBYFMTChange.Attributes("onclick") = "var ok=window.confirm('" + "是否開啟Excel格式轉換或準備BYFCT資料？" + "');if(!ok){return false;} else {CheckAttribute('CBBYFMTChange')}"
        BBYImport.Attributes("onclick") = "var ok=window.confirm('" + "是否開啟Excel準備資料？" + "');if(!ok){return false;} else {CheckAttribute('CBBYImport')}"
    ...
    Sub SetCustomer()
        ...
        If dt_ControlRecord.Rows.Count > 0 Then
            ...
            If uFASCommon.UpdateControlRecord(DBuyer.Text, xUserID, "StartBY", 0, "StartBY", 0) = 0 Then
                ...
                ' 準備 BY FCT 資料
                If xOffice2010 = 1 Then
                    DFileName.Text = xCustomerName + "_BYImport.xlsm"
                Else
                    DFileName.Text = xCustomerName + "_BYImport.xls"
                End If
    ...
    Protected Sub BBYFMTChange_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles BBYFMTChange.Click
        ...
        If uFASCommon.CheckControlRecord(DBuyer.Text, "BYFMTChange", 2) = 0 Then
            ...
            If AtGoImport.Checked = True Then
