...
Partial Class InfF_StockProdFreeList
    ...
    Protected Sub BInq_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BInq.Click
        ...
        '
        sql = "SELECT Rtrim(ITMCDE) As Item, Rtrim(CLRCDE) As Color, '' As KeepCode, "
        sql &= "'' As ItemName, "
        ...
        sql &= "WHERE DPTCDE = '" & "01" & "' "
        If Buyer = "000013T" Then
            sql &= "  AND ITMCDE = '" & DItem.Text & "' "
        Else
            If DItem.Text <> "" Then
                sql &= "  AND ITMCDE = '" & DItem.Text & "' "
            End If
        End If
        sql &= "GROUP BY ITMCDE, CLRCDE "
        sql &= "ORDER BY ITMCDE, CLRCDE "
        '
        Dim DBAdapter1 As New OleDbDataAdapter(sql, cn)
        DBAdapter1.Fill(ds, "TDE00")
        If ds.Tables("TDE00").Rows.Count > 0 Then
            GridView1.Visible = True
            GridView1.DataSource = ds.Tables("TDE00")
            GridView1.DataBind()
        End If
        '
        cn.Close()
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        If (e.Row.RowType = DataControlRowType.DataRow) Then
            ...
            sql = "SELECT Ltrim(Rtrim(SLCCDE)) AS LOC, SFRQDE AS QTY, LSCUDE AS LASTTIME FROM WAVEDLIB.TDE00 "
            sql &= "Where DPTCDE = '" & "01" & "' "
            sql &= "  And ITMCDE = '" & DataBinder.Eval(e.Row.DataItem, "Item") & "' "
            sql &= "  And CLRCDE = '" & DataBinder.Eval(e.Row.DataItem, "Color") & "' "
            sql &= "Order By SLCCDE "
            '
            Dim DBAdapter1 As New OleDbDataAdapter(sql, cn)
