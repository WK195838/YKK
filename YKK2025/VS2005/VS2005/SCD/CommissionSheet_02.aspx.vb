...
Partial Class CommissionSheet_02
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        '檢查起單日期是否在指定日期之後
        Dim SQL As String
        'SQL = " Select convert(char(10),appdate,112) appdate From F_CommissionSheet "
        'SQL = SQL + " where formsno = '" + CStr(wFormSno) + "'"
        'SQL = SQL + " and  convert(char(10),appdate,112) >= (select data from m_referp where dkey = 'appdate')"
        SQL = "  Select A.NO,'OLD' AS DATA  From F_CommissionSheet A,FS_GentaniSheet B"
        SQL = SQL + " where A.NO =B.NO AND formsno  = '" + CStr(wFormSno) + "'"
        SQL = SQL + " UNION ALL"
        SQL = SQL + " Select A.NO,'NEW' AS DATA  From F_CommissionSheet A,FS_GentaniSheetNEW B"
        SQL = SQL + " where A.NO =B.NO AND formsno  = '" + CStr(wFormSno) + "'"
        Dim dtData As DataTable = uDataBase.GetDataTable(SQL)
        If dtData.Rows.Count > 0 Then
            If dtData.Rows(0).Item("DATA") = "NEW" Then
                '如果是就執行新程式
                Dim URL As String = "CommissionSheet_04.aspx?pFormNo=002002&pFormSno=" & CStr(wFormSno)
    ...
    Sub ShowFormData()
        ...
        If wFormSno >= 5001 Then
            LOldCSheet.Visible = False
            LOldMSheet.Visible = False
            LOldSImage.Visible = False
            LOldSSheet.Visible = False
        End If
        '
        Dim SQL As String
        ...
        If dtCommissionSheet.Rows.Count > 0 Then
            ...
            '20240814 增加訂止受注連結
            If dtCommissionSheet.Rows(0).Item("Sts_Norder") = 1 Then
                NorderLink.Visible = True
                NorderLink.NavigateUrl = dtCommissionSheet.Rows(0).Item("Sts_NorderURL")

            Else
                NorderLink.Visible = False
            End If
            ...
            '----舊式委託-------------------------------------------------  
            If wFormSno < 5001 Then
                LOldCSheet.Visible = True
                LOldCSheet.NavigateUrl = uCommon.GetAppSetting("HttpOld") & "ComSheet.asp?DRNO=" + dtCommissionSheet.Rows(0).Item("NO")
            End If
            '----基本欄位設定-------------------------------------------------  
            ...
            ''舊式委託??
            'If wFormSno < 5001 Then
            '    SQL = SQL + "Order by Step Desc, Unique_ID "
            'Else
            ...
            SQL = SQL + "FROM ( "
            SQL = SQL + " select  FormNo, FormSno, Step, SeqNo, StsDesc, FormName, FlowTypeDesc, DecideName, DelaySts, StepNameDesc, "
            SQL = SQL + " DecideDescA, AgentName, '' as Description, BStartTimeDesc, BEndTimeDesc, AStartTimeDesc, AEndTimeDesc, "
            SQL = SQL + " WorkID, '' As ViewOP, '' AS URL, Active,CreateTime from V_WaitHandle_01 "
            SQL = SQL + " union all "
            SQL = SQL + " select FormNo, FormSno, Step, SeqNo, StsDesc, FormName, FlowTypeDesc, DecideName, DelaySts, StepNameDesc, "
            SQL = SQL + " DecideDescA, AgentName, '' as Description, BStartTimeDesc, BEndTimeDesc, AStartTimeDesc, AEndTimeDesc, "
            SQL = SQL + " WorkID, '' As ViewOP, '' AS URL, Active,CreateTime from  V_WaitHandle_old_01)a"
            SQL = SQL + " Where FormNo  = '" & wFormNo & "' "
            SQL = SQL + " And FormSno = '" & CStr(wFormSno) & "' "
            ...
            If dt_WaitHandle.Rows.Count > 0 Then
                For i As Integer = 0 To dt_WaitHandle.Rows.Count - 1
                    ' StepNameDesc(9)
                    Dim a As String = dt_WaitHandle.Rows(i)(2).ToString
