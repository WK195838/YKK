...
Partial Class StockList
    ...
    Sub GetData()
        ...
        If DBDataSet1.Tables("GetData").Rows.Count > 0 Then
            ...
            (Mid(DBDataSet1.Tables("GetData").Rows(0).Item("WSHCXA"), 1, 4) = "OVER")) And wStep = "" Then
                ...
                SQL = SQL + " Case when left(WSHCXA,2) ='PO' then 'ITEM+數量' when  left(WSHCXA,3) in('DYE','BOX') OR  left(WSHCXA,4) ='OVER' then 'ITEM+板數' else '板號' end as Type "
    ...
    Protected Sub BAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BAdd.Click
        ...
        If cun <> Int(wqty) And wqty <> "0" Then
            Label1.Text = "申請者需要板數=" + wqty + ",選取的板數=" + Str(cun) + "數量不合！"
        Else
            If wStep = "" Then
                wType = "板號"
            Else
