...
Partial Class AddItemListCL
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Depo = Request.QueryString("pDepo")

        NowDateTime = Now.ToString("yyyyMMddHHmmss")



        BExpItemList.Attributes.Add("onClick", "GetExpItem('" + Depo + "')")
        '
        ...
        If Not Me.IsPostBack Then   '不是PostBack
            ...
            'If Len(DTNo) <> 14 Then
            Dim sql As String
            sql = " delete from F_FundingSheetCLdttemp  where dtno ='" + DTNo + "'"
            uDataBase.ExecuteNonQuery(sql)

            sql = " insert into F_FundingSheetCLdttemp (dtno,expcat,expitem,acid,adate,taxtype,netamt,taxamt,amt,content,remark,InvoiceNo,TaxNo,GUINo,TaxBase,createuser,createtime)"
            sql = sql + " Select  no,expcat,expitem,acid,adate,taxtype,netamt,taxamt,amt,content,remark,InvoiceNo,TaxNo,GUINo,TaxBase,createuser,createtime  from  F_FundingSheetCLdt "
    ...
    Protected Sub BADD_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles BADD.ServerClick
        If InputDataOK(0) Then
            ...
            sql = " Insert into F_FundingSheetcldttemp (DTNo, ExpCat, ExpItem,ACID, ADate, TaxType, NetAmt, TaxAmt, Amt, Content, Remark,InvoiceNo,TaxNo,GUINo,TaxBase,Createuser,CreateTime,ModifyUser,ModifyTime) "
            sql = sql & "VALUES( "
            sql = sql & " '" & DTNo & "', "
    ...
    Sub GetData()
        ...
        SQL = " Select  Unique_ID,ExpCat+'--'+ExpItem ExpItem,ACID, ADate, TaxType, NetAmt, TaxAmt, Amt, Content, Remark,case when InvoiceNo ='' then Taxno else InvoiceNo end as InvoiceNo,GUINo,TaxBase from  F_FundingSheetcldttemp  where 1=1 "
    ...
    Protected Sub GridView1_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles GridView1.RowDeleting
        ...
        SQL = " delete  from  F_FundingSheetcldttemp "
    ...
    Protected Sub BClose_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles BClose.ServerClick
        ...
        'jessica 2021/2/5
        sql = "Select * From F_FundingSheetcldttemp "
        sql = sql & "Where DTNO = '" + DTNo + "' "
        Dim dtTemp As DataTable = uDataBase.GetDataTable(sql)
        If dtTemp.Rows.Count > 0 Then
            '將登錄的細項INERT 到DT 
            sql = " delete from F_FundingSheetCLdt  where no ='" + DTNo + "'"
            uDataBase.ExecuteNonQuery(sql)

            sql = " insert into F_FundingSheetCLdt (no,expcat,expitem,acid,adate,taxtype,netamt,taxamt,amt,content,remark,InvoiceNo,TaxNo,GUINo,TaxBase,createuser,createtime)"
            sql = sql + " Select  dtno,expcat,expitem,acid,adate,taxtype,netamt,taxamt,amt,content,remark,InvoiceNo,TaxNo,GUINo,TaxBase,createuser,createtime  from  F_FundingSheetcldttemp "
    ...
    Sub SetFieldData()
        ...
        Dim SQL As String
        Dim i As Integer
        SQL = "  Select taxname+'('+taxid+')' as TaxName,TaxP from m_taxListCL order by taxname  "
