...
Partial Class BatchAutoApproveDye
    ...
    Sub InsertBatchTable() '新增資料至批次轉檔
        ...
        Sql = Sql + " formno in('005011','005015','005016')  and Active = '1'   And (Sts = '0'  Or  Sts = '4')  and step  in (51,52,53) "
        ...
        Sql = Sql + " where  formno in('005011','005015','005016') and Active = '1'   And (Sts = '0'  Or  Sts = '4') and step  in (51,52,53)  "
        uDataBase.ExecuteNonQuery(Sql)
    ...
    Sub FlowControl()
        ...
        SQL = SQL & " Where formno in('005011','005015','005016') "
        ...
        For i As Integer = 0 To dtFlow.Rows.Count - 1
            ...
            If dtWaitHandle.Rows.Count > 0 Then
                ...
                SQL = SQL + " where formno in('005011','005015','005016') and "
            ...
            While Run = True
                ...
                If ErrCode = 0 Then
                    If RepeatRun = False Then   '不是通知的重覆執行
                        ...
                        '檢查是否都完成才送至60
                        Dim SQL1 As String
                        If pFormNo = "005011" Or pFormNo = "005016" Then
                            If pStep = 51 Then
                                SQL1 = " Select step From T_WaitHandle Where "
                                SQL1 = SQL1 + "    FormNo  in('005011','005015','005016')   And FormSno = '" + CStr(pFormSno) + "'   And Step = '52'  and  modifyuser <> 'Operation_02'"
                            ...
                            If pStep = 52 Then
                                SQL1 = " Select step From T_WaitHandle Where  "
                                SQL1 = SQL1 + "    FormNo   in('005011','005015','005016')   And FormSno = '" + CStr(pFormSno) + "'   And Step = '51'  and  modifyuser <> 'Operation_02'"
                                SQL1 = SQL1 + " and  formno ='" + pFormNo + "'"
                                Dim dtFlow As DataTable = uDataBase.GetDataTable(SQL1)
                                If dtFlow.Rows.Count > 0 Then
                                    pRunNextStep = 1
                        ...
                        Else
                            ...
                            If pStep = 53 Then
                                SQL1 = " Select step From T_WaitHandle Where  "
                                SQL1 = SQL1 + "    FormNo   in('005011','005015','005016')   And FormSno = '" + CStr(pFormSno) + "'   And Step in( '51','52')  and  modifyuser <> 'Operation_02'"
                                SQL1 = SQL1 + " and  formno ='" + pFormNo + "'"
                                Dim dtFlow As DataTable = uDataBase.GetDataTable(SQL1)
                                If dtFlow.Rows.Count > 0 Then
                                    pRunNextStep = 1
    ...
    Function ModifyData(ByVal pFun As String, _
        ...
        Dim RtnCode As Integer = 0
        Dim sql As String
        sql = "Update f_newcoloruakipling Set "
        sql &= " Sts = '" & pSts & "',"
