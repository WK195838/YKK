...
Partial Class DTMW_NewColorSheet01_03CFBP14
    ...
    Sub SetParameter()
        ...
        If wStep = 10 Or wStep = 510 Or wStep = 20 Or wStep = 25 Or wStep = 520 Then '指定徐慧芬
            Dim SQL As String
        ...
        Response.Cookies("PGM").Value = "DTMW_NewColorSheet01_03CFBP14.aspx"
    ...
    Sub ShowFormData()
        ...
        Dim SQL As String
        SQL = "Select * From F_NewColor03CFBP14 "
        SQL = SQL & " Where FormNo =  '" & wFormNo & "'"
        SQL = SQL & "   And FormSno =  '" & CStr(wFormSno) & "'"
        Dim DBAdapter1 As DataTable = uDataBase.GetDataTable(SQL)
    ...
    Sub SetFieldAttribute(ByVal pPost As String)
        ...
        SQL = SQL + " where   dkey  = 'Sheet-" + wFormNo + "'"
        Dim DBData1 As DataTable = uDataBase.GetDataTable(SQL)
        LSheetName.Text = DBData1.Rows(0).Item("Data")
        ...
        End Select
        If pPost = "New" Then DCustomerCheck.Checked = False
        ...
        'OverseaYKKCode
        Select Case FindFieldInf("Recolorcode")
            Case 0  '顯示
                DRecolorcode.BackColor = Color.LightGray
                DRecolorcode.ReadOnly = True
                DRecolorcode.Visible = True
            Case 1  '修改+檢查
                DRecolorcode.BackColor = Color.GreenYellow
                ShowRequiredFieldValidator("DRecolorcodeRqd", "DRecolorcode", "異常：需輸入對應YKK色號")
                DRecolorcode.Visible = True

            Case 2  '修改
                DRecolorcode.BackColor = Color.Yellow
                DRecolorcode.Visible = True
            Case Else   '隱藏
                DRecolorcode.Visible = False
        End Select
        If pPost = "New" Then SetFieldData("Recolorcode", "ZZZZZZ")
    ...
    Sub FlowControl(ByVal pFun As String, ByVal pAction As Integer, ByVal pSts As String)
        ...
        If ErrCode = 0 Then
            If DNo.Text <> "" Then
                ErrCode = oCommon.CommissionNo("005003", wFormSno, wStep, DNo.Text) '表單號碼, 表單流水號, 工程, 委託書No
                If ErrCode <> 0 Then
                    ErrCode = 9060
                End If
            End If
        ...
        If ErrCode = 0 Then
            ...
            While Run = True
                ...
                If ErrCode = 0 And pRunNextStep = 1 Then
                    ...
                    If (wStep = 10 Or wStep = 510) And pFun = "OK" Then
                        If (DVCACheck.Checked = True) Or (DFactoryCheck.Checked = True) Then
                            pAction = 2
                        Else
                            pAction = 0
                        End If
                    ...
                    If wStep = 40 And pAction = 0 And DNewOldColor.SelectedValue <> "舊色" Then
                        pAction = 3
                    End If

                    If wStep = 70 And pAction = 0 And DYKKColorType.SelectedValue <> "螢光" Then
                        pAction = 2
                    End If
        ...
        End If      '儲存表單ErrCode=0
            ...
            If ErrCode = 0 Then
                ...
                Else
                    If ((LastStep = 20 Or LastStep = 520 Or LastStep = 25 Or LastStep = 525 Or LastStep = 31) And pFun = "OK") Or ((LastStep = 25 Or LastStep = 520 Or LastStep = 525 Or LastStep = 31) And pFun = "NG2") Then '新色確認完成表按OK才進來

                        Dim SQL As String  '檢查是否有相同版本的新色依賴完成書
                        SQL = " select  *  from F_NewColor03CFBP14 a, F_NewColorComplete b"
                        ...
                        If dtFlow.Rows.Count = 0 Then '如果沒有才可以新增
                            URL = uCommon.GetAppSetting("NewColorCompleteUrl") & "?pFormNo=" & wFormNo & "&pFormSno=" & wFormSno & "&pStep=" & wStep & "&pNextGate=" & wNextGate & _
                    ...
                    ElseIf (LastStep = 60) And pFun = "OK" Then '新色再現完成表按OK才進來
                        ...
                        Dim dtFlow As DataTable = uDataBase.GetDataTable(SQL)
                        If dtFlow.Rows.Count = 0 Then '如果沒有才可以新增
                            URL = uCommon.GetAppSetting("NewColorAgainUrl") & "?pFormNo=" & wFormNo & "&pFormSno=" & wFormSno & "&pStep=" & wStep & "&pNextGate=" & wNextGate & _
                                                                                                       "&pUserID=" & wUserID & "&pApplyID=" & wApplyID
                        Else
                            URL = uCommon.GetAppSetting("MessageUrl") & "?pMSGID=C&pFormNo=" & wFormNo & "&pStep=" & wStep & "&pNextGate=" & wNextGate & _
                                                                                   "&pUserID=" & wUserID & "&pApplyID=" & wApplyID
                        End If
                    Else
                        If (LastStep = 85 Or LastStep = 80) And wStep = 0 Then '會簽
                            wStep = 90
                        End If
                        URL = uCommon.GetAppSetting("MessageUrl") & "?pMSGID=C&pFormNo=" & wFormNo & "&pStep=" & wStep & "&pNextGate=" & wNextGate & _
                                                                                   "&pUserID=" & wUserID & "&pApplyID=" & wApplyID
    ...
    Sub AppendData(ByVal pFun As String, ByVal NewFormSno As Integer)
        ...
        Dim SQl As String
        SQl = "Insert into F_NewColor03CFBP14 "
        SQl = SQl + "( "
        ...
        SQl = SQl + "BuyerCode,CustomerColor,CustomerColorCode,OverseaYKKCode,ReColorCode,PANTONECode,"  '11~15
        SQl = SQl + "DevYear,DevSeason,ReceiveDate,ColorLight1,ColorLight2,NewOldColor," ' 16~20
        ...
        SQl = SQl + "YKKColorType,YKKColorCode,Version,CheckNo,SLDColor,Again," '31~35
        SQl = SQl + "VFCOLOR," '36
        ...
        SQl = SQl + " N'" + YKK.ReplaceString(DReColorCode.Text.ToUpper) + "', "                ' 海外YKK色號14
        SQl = SQl + " N'" + YKK.ReplaceString(DPANTONECode.Text.ToUpper) + "', "
    ...
    Sub ModifyData(ByVal pFun As String, ByVal pSts As String)
        ...
        SQL = SQL + " ReColorCode = N'" & YKK.ReplaceString(DReColorCode.Text.ToUpper) & "',"
        ...
        If wStep = 90 And DNewOldColor.SelectedValue = "新色" Then 'Master 建檔後轉資料到WINS COLOR STRUCTURE
            ...
            SQL = SQL + " where sts in (1,0) and no = '" + DNo.Text + "' AND TYPE ='N1'"
            SQL = SQL + " )a"
            uDataBase.ExecuteNonQuery(SQL)
        ...
        OleDbConnection1.ConnectionString = System.Configuration.ConfigurationManager.AppSettings("SqlConnWAVES")  'SQL連結設定


        '20240130 jessica 舊色型別轉換
        If wStep = 40 And DNewOldColor.SelectedValue = "舊色" And DYKKColorCode.Value <> "" Then

            Dim snum, pnum As String
            snum = DYKKColorCode.Value
            pnum = snum.PadLeft(5, " ")

            ''舊色
            'SQL = "SELECT *  FROM MST_COLOR "
            'SQL = SQL + " where CLRCA2 = '" + DYKKColorCode.Value + "'"
            ''SQL = SQL + " AND ITPCA2  ='PBR2D'"
            'OleDbConnection1.Open()
            'Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
            'DBAdapter1.Fill(DBDataSet1, "MST_COLOR")
            'DBTable1 = DBDataSet1.Tables("MST_COLOR")
            'If DBTable1.Rows.Count = 0 Then

            SQL = " insert into R_NCFA200WK(NO,PDPCA2,STS,PCCCA2,CLRCA2,SIZCA2,FMLCA2,ITPCA2,PCACA2,ST1CA2,ST2CA2,ST3CA2,ST4CA2,ST5CA2,ST6CA2,ST7CA2,UIDCA2,PRGCA2,DEVCA2,RADUA2,RADTA2,RUPUA2,RUPTA2,RCMCA2)"
            SQL = SQL + " select '" + DNo.Text + "' AS NO,'01',0,PCCCA2,'" + pnum + "' as CLRCA2,SIZCA2,FMLCA2,ITPCA2,PCACA2,'','','','','','','','','','',"
            SQL = SQL + " CONVERT(CHAR(10),GETDATE(),112),"
            SQL = SQL + " SUBSTRING(CONVERT(CHAR(10),GETDATE(),108),1,2)+SUBSTRING(CONVERT(CHAR(10),GETDATE(),108),4,2)+SUBSTRING(CONVERT(CHAR(10),GETDATE(),108),7,2),"
            SQL = SQL + "   '','','' from R_NewColorST_02 where ITPCA2   in ('BP14','BP16') "
            uDataBase.ExecuteNonQuery(SQL)

            Code = oWaves.OldColor2Wings(DNo.Text)
    ...
    Function SetNo(ByVal Seq As Integer) As String
        ...
        'Next
        'Str = Str + Str2
        Str = "C" + CStr(DateTime.Now.Year) + Str2
        '年
        '當月份筆數有幾筆  20150414 Modify by Jessica
        Dim sql As String
        sql = " select count(*)cun from  f_Newcolor03cfbp14"
    ...
    Function OK() As Boolean
        ...
        If wStep = 30 Or wStep = 25 Or wStep = 525 Then
        ...
        Dim OverSeaYKK As String = 0
        '檢查海外ykk 限定勾選工廠自核

        SQL = " select * from  mst_custmer"
        SQL = SQL + " where cust_type = 'k'"
        SQL = SQL + " and custmer = '" + DCustomerCode.Text + "'"
        Dim dt1 As DataTable = uDataBase.GetDataTable(SQL)
        If dt1.Rows.Count > 0 Then
            OverSeaYKK = 1
            ...
            If DOverseaYKKCode.Text = "" Then
                ...
                DOverseaYKKCode.ReadOnly = False
                DOverseaYKKCode.BackColor = Color.GreenYellow
            End If
        Else
        ...
        If wStep = 1 Or wStep = 500 Then
            ...
            'jessica 20210201 怡蓉追加
            If DReColorCode.Text <> "" Then
                Dim NoStr As String = ""

                SQL = " select b.*,a.Recolorcode from ( "
                SQL = SQL + " select no, Recolorcode from  v_newcolor"
                SQL = SQL + " where(sts <> 2)"
                SQL = SQL + " )a,v_newcolor b "
                SQL = SQL + " where a.no = b.no and  (a.Recolorcode='" + DReColorCode.Text + "' or  ykkcolorcode ='" + DReColorCode.Text + "')"
                SQL = SQL + " and formno = '" + wFormNo + "'"
                ...
                Dim DBAdapter2 As DataTable = uDataBase.GetDataTable(SQL)
                If DBAdapter2.Rows.Count > 0 Then
                    For Each dtr As Data.DataRow In DBAdapter2.Rows
                        If NoStr = "" Then
                            NoStr = dtr("No")
                        Else
                            NoStr = NoStr + "," + dtr("No")
        ...
        '檢查BUYER：Adidas/Reebok/Taylormade/Nike/Puma 限定勾選VCA
        If OverSeaYKK = 0 Then

            If DOverseaYKKCode.Text <> "" Then
                isOK = False
                Message = "異常：非海外YKK不可填海外YKK色號!"
                DOverseaYKKCode.Text = ""
                DOverseaYKKCode.BackColor = Color.Yellow
            End If

            SQL = " Select  * from  M_referp "
            SQL = SQL + " where Cat = '5001' and dkey = 'VCABUYER' "
            SQL = SQL + " and data = '" + DBuyer.Text + "'"
            ...
            If dt.Rows.Count > 0 Then
                ...
                If DVCACheck.Checked = False Then
                    ...
                    isOK = False
                    Message = Message + "異常：BUYER：Adidas/Taylormade/Nike/Puma 限定勾選VCA!"
                End If
            End If
        End If
        ...
        Dim Q As Integer = 0   'jessica 20150326
        If wStep = 40 Then
            '檢查是否有Q 
        ...
        '檢查是否有輸入廢止色 20240717 Hower
        If wStep = 1 Or wStep = 500 Then
            Dim Code As String
            Dim CodeStr As String

            Code = DReColorCode.Text

            CodeStr = "廢止色不可輸入"

            SQL = " select * from F_combirepeal"
            SQL = SQL + " where  1=1 and colorcode ='" + Code + "'"
            Dim DBAdapter3 As DataTable = uDataBase.GetDataTable(SQL)
            If DBAdapter3.Rows.Count > 0 Then
    ...
    Sub CheckDuplicateNo()
        ...
        Dim SQL As String
        SQL = "select no,customer,customercode,buyer,buyercode,customercolor,customercolorcode,overseaykkcode,pantonecode devyear,devseason "
        SQL = SQL + " from F_NewColor03CFBP14"
        SQL = SQL + " where  sts <>2 and  customercode='" + DCustomerCode.Text + "'"
        SQL = SQL + " and  buyercode='" + DBuyerCode.Text + "'"
        SQL = SQL + " and customercolor='" + DCustomerColor.Text + "'"
        SQL = SQL + " and  customercolorcode='" + DCustomerColorCode.Text + "'"
        SQL = SQL + " and overseaykkcode='" + DOverseaYKKCode.Text + "'"
        SQL = SQL + " and pantonecode='" + DPANTONECode.Text + "'"
        SQL = SQL + " and  devyear='" + DDevYear.SelectedValue + "'"
        SQL = SQL + " and devseason='" + DDevSeason.SelectedValue + "'"
