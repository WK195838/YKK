...
Partial Class DTMW_NewColorSheet01_VB
    ...
    Sub SetParameter()
        ...
        If wStep = 10 Or wStep = 510 Or wStep = 511 Or wStep = 20 Or wStep = 520 Or wStep = 521 Then '指定徐慧芬
            Dim SQL As String
    ...
    Sub ShowFormData()
        ...
        If DBAdapter1.Rows.Count > 0 Then
            ...
            If wStep = 20 Or wStep = 520 Or wStep = 521 Then
                Formname = "3CF DTM"
            ...
            If wStep = 520 Or wStep = 521 Then
                Version = Version + 1
            End If
    ...
    Sub SetFieldAttribute(ByVal pPost As String)
        ...
        If wFormNo = "005014" Then
            LSheetName.Text = "拉鏈ZIPPER CHAIN (VB)" '表單名稱
        End If
        ...
        Select Case FindFieldInf("Buyer")
            ...
            Case Else   '隱藏
                ...
                BBuyer.Visible = False
        End Select
        If pPost = "New" Then DBuyer.Text = "VERA BRADLEY"
        ...
        End Select
        If pPost = "New" Then DBuyerCode.Text = "TW0655"
    ...
    Sub FlowControl(ByVal pFun As String, ByVal pAction As Integer, ByVal pSts As String)
        ...
        If ErrCode = 0 Then
            If DNo.Text <> "" Then
                ErrCode = oCommon.CommissionNo("005014", wFormSno, wStep, DNo.Text) '表單號碼, 表單流水號, 工程, 委託書No
                If ErrCode <> 0 Then
                    ErrCode = 9060
                End If
            End If
        ...
        If ErrCode = 0 Then
                    ...
                    Else
                        If RepeatRun = False Then   '不是通知的重覆執行
                            ...
                            If wStep = 51 Then
                                ...
                                SQL1 = SQL1 + "    FormNo  = '005014'   And FormSno = '" + CStr(wFormSno) + "'   And Step = '52'  "
                            ...
                            If wStep = 52 Then
                                SQL1 = " Select step From T_WaitHandle Where  "
                                SQL1 = SQL1 + "    FormNo  = '005014'   And FormSno = '" + CStr(wFormSno) + "'   And Step = '51'  "
                                Dim dtFlow As DataTable = uDataBase.GetDataTable(SQL1)
                                If dtFlow.Rows.Count > 0 Then
                                    pRunNextStep = 1
                                Else
            ...
            If ErrCode = 0 Then
                ...
                Else
                    ...
                    If ((wbStep = 20 Or wbStep = 520 Or wbStep = 521) And pFun = "OK") Then '新色確認完成表按OK才進來
                        Dim FormName As String = ""
                        If LastStep = 20 Or LastStep = 520 Or LastStep = 521 Then
                            FormName = "3CF DTM"
                        ...
                        SQL = " select  *  from  F_NewColorVB a, F_NewColorComplete b"
                        ...
                        If dtFlow.Rows.Count = 0 Then '如果沒有才可以新增
                            URL = uCommon.GetAppSetting("NewColorCompleteUrl") & "?pFormNo=" & wFormNo & "&pFormSno=" & wFormSno & "&pStep=" & LastStep & "&pNextGate=" & wNextGate & _
    ...
    Sub ModifyData(ByVal pFun As String, ByVal pSts As String)
        ...
        If wStep = 85 And DVFColor1.Text.ToUpper <> "" Then
    ...
    Function SetNo(ByVal Seq As Integer) As String
        ...
        'Next
        'Str = Str + Str2
        Str = "M" + CStr(DateTime.Now.Year) + Str2
        '年
        'Set單號
        '當月份筆數有幾筆  20150414 Modify by Jessica
        SQL = " select   isnull(max(convert(int,substring(no,8,4))),0) cun from   F_NewColorVB"
    ...
    Function OK() As Boolean
        ...
        If wStep = 20 Or wStep = 521 Or wStep = 520 Then
        ...
        If DBuyerCode.Text <> "TW0655" Then
            isOK = False
            Message = "異常：BUYER 請輸入VERA BRADLEY!"
        End If
