...
Partial Class DTMW_NewColorSheet01_TNF
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "DTMW_NewColorSheet01_TNF.aspx"
    ...
    Sub SetFieldAttribute(ByVal pPost As String)
        ...
        If wFormNo = "005015" Then
            LSheetName.Text = "拉鏈ZIPPER CHAIN (TNF)" '表單名稱
        End If
    ...
    Sub FlowControl(ByVal pFun As String, ByVal pAction As Integer, ByVal pSts As String)
        ...
        If ErrCode = 0 Then
            If DNo.Text <> "" Then
                ErrCode = oCommon.CommissionNo("005015", wFormSno, wStep, DNo.Text) '表單號碼, 表單流水號, 工程, 委託書No
                If ErrCode <> 0 Then
                    ErrCode = 9060
                End If
            End If
            ...
            While Run = True
                    ...
                    Else
                        If RepeatRun = False Then   '不是通知的重覆執行
                            ...
                            If wStep = 51 Then
                                ...
                                SQL1 = SQL1 + "    FormNo  = '005015'   And FormSno = '" + CStr(wFormSno) + "'   And Step = '52'   and  modifyuser <> 'Operation_02'  "
                            ...
                            If wStep = 52 Then
                                SQL1 = " Select step From T_WaitHandle Where  "
                                SQL1 = SQL1 + "    FormNo  = '005015'   And FormSno = '" + CStr(wFormSno) + "'   And Step = '51'  and  modifyuser <> 'Operation_02'  "
                                Dim dtFlow As DataTable = uDataBase.GetDataTable(SQL1)
                                If dtFlow.Rows.Count > 0 Then
                                    pRunNextStep = 1
                                Else
                ...
                If ErrCode = 0 And pRunNextStep = 1 Then
                    ...
                    If (wStep = 30 Or wStep = 530) And pFun = "OK" Then
                        If DBuyerCode.Text = "000021" Then 'TNF
                            pAction = 0
                        End If
                    End If
            ...
            If ErrCode = 0 Then
                ...
                Else
                    ...
                    If ((wbStep = 20 Or wbStep = 520) And pFun = "OK") Or ((wbStep = 30 Or wbStep = 530) And pFun = "OK") Then '新色確認完成表按OK才進來
                        ...
                        SQL = " select  *  from F_NewColorTNF a, F_NewColorComplete b"
    ...
    Function SetNo(ByVal Seq As Integer) As String
        ...
        'Next
        'Str = Str + Str2
        Str = "N" + CStr(DateTime.Now.Year) + Str2
        '年
        'Set單號
        '當月份筆數有幾筆  20150414 Modify by Jessica
        SQL = " select   isnull(max(convert(int,substring(no,8,4))),0) cun from  F_NewColorTNF"
    ...
    Function OK() As Boolean
        ...
        If DBuyerCode.Text <> "000021" Then
            isOK = False
            Message = "異常：BUYER 請輸入TNF!"
        End If
