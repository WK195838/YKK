...
Partial Class HR_WorkTime_01
    ...
    '++ 工時
    '+++++++++++++++++++++++++++++++++++++++++++++++
    Dim SumWTBase As Double = 0       '合計基本 
    Dim SumWTOverTime As Double = 0   '合計加班 
    Dim SumWTTimeOff As Double = 0    '合計請假 
    Dim SumWTKousu1 As Double = 0     '合計工時(分) 
    Dim SumWTKousu2 As Double = 0     '合計工時(率) 
    '+++++++++++++++++++++++++++++++++++++++++++++++
    ...
    Sub SetParameter()
        ...
        'Add-Start by Joy 2009/11/20(2010行事曆對應)
        wDepo = oCommon.GetCalendarGroup(Request.Cookies("UserID").Value)
    End Sub
    ...
    Sub DataList()
        ...
        SQL = SQL + "'' As WTBase, '' As WTOverTime, '' As WTTimeOff, '' As WTMin, '' As WTPer, DepoID, HRWDivID "
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            '++  工時處理
            '++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            Dim wBase As Integer = 0
            Dim wOverTime As Integer = 0
            Dim wTimeOff As Integer = 0
            Dim wATime As Integer = 0
            Dim wBTime As Integer = 0
            '基本工時
            If e.Row.Cells(4).Text = "" Or e.Row.Cells(4).Text = ":" Or e.Row.Cells(4).Text = "&nbsp;" Or _
               e.Row.Cells(5).Text = "" Or e.Row.Cells(5).Text = ":" Or e.Row.Cells(5).Text = "&nbsp;" Then
                wATime = 0
                wBTime = 0
            Else
                wATime = CInt(Mid(e.Row.Cells(4).Text, 1, 2)) * 60 + CInt(Mid(e.Row.Cells(4).Text, 4, 2))
                wBTime = CInt(Mid(e.Row.Cells(5).Text, 1, 2)) * 60 + CInt(Mid(e.Row.Cells(5).Text, 4, 2))
            End If
            wBase = wBTime - wATime
            'MsgBox(CStr(wBase) + ":" + CStr(wATime) + ":" + CStr(wBTime))
            If wBase > 0 Then
                If Mid(e.Row.Cells(23).Text, 1, 3) = "110" Or Mid(e.Row.Cells(23).Text, 1, 3) = "111" Then
                    If CInt(Mid(e.Row.Cells(4).Text, 1, 2)) <= 12 Then
                        wBase = wBase - 60
                    End If
                    If e.Row.Cells(22).Text = "10" Then
                        If wBase > 480 Then wBase = 480
                    End If
                Else
                    If CInt(Mid(e.Row.Cells(4).Text, 1, 2)) <= 12 Then
                        wBase = wBase - 45
                ...
                End If
            End If
            'MsgBox(e.Row.Cells(2).Text + ":" + e.Row.Cells(4).Text + ":" + e.Row.Cells(5).Text + ":" + CStr(wBase))

            '加班
            If CDbl(e.Row.Cells(11).Text.ToString) > 0 Then
                wOverTime = CDbl(e.Row.Cells(11).Text.ToString) * 60
            End If
            '特休
            If CDbl(e.Row.Cells(12).Text.ToString) > 0 Then
                DBDataSet1.Clear()
                ...
                SQL = SQL + "Where Sts = '1' "
                SQL = SQL + "  And Name = '" + DName.SelectedValue + "'"
                SQL = SQL + "  And VacationCode = 'A' "
                SQL = SQL + "  And AStartDate <= '" + e.Row.Cells(2).Text.ToString + "'"
                SQL = SQL + "  And AEndDate   >= '" + e.Row.Cells(2).Text.ToString + "'"
                Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
                DBAdapter1.Fill(DBDataSet1, "Vacation")
                If DBDataSet1.Tables("Vacation").Rows.Count > 0 Then
                    If DBDataSet1.Tables("Vacation").Rows(0).Item("AStartDate") < e.Row.Cells(2).Text.ToString And _
                       DBDataSet1.Tables("Vacation").Rows(0).Item("AEndDate") > e.Row.Cells(2).Text.ToString Then
                        wTimeOff = 480
                    Else
                        If DBDataSet1.Tables("Vacation").Rows(0).Item("AStartDate") = e.Row.Cells(2).Text.ToString And _
                           DBDataSet1.Tables("Vacation").Rows(0).Item("AEndDate") = e.Row.Cells(2).Text.ToString Then
                            If DBDataSet1.Tables("Vacation").Rows(0).Item("AEndH") - DBDataSet1.Tables("Vacation").Rows(0).Item("AStartH") > 4 Then
                                wTimeOff = (DBDataSet1.Tables("Vacation").Rows(0).Item("AEndH") - DBDataSet1.Tables("Vacation").Rows(0).Item("AStartH") - 1) * 60
                            Else
                                wTimeOff = (DBDataSet1.Tables("Vacation").Rows(0).Item("AEndH") - DBDataSet1.Tables("Vacation").Rows(0).Item("AStartH")) * 60
                            End If
                        Else
                            If DBDataSet1.Tables("Vacation").Rows(0).Item("AStartDate") = e.Row.Cells(2).Text.ToString Then
                                If DBDataSet1.Tables("Vacation").Rows(0).Item("AStartH") < 12 Then
                                    wTimeOff = (17 - DBDataSet1.Tables("Vacation").Rows(0).Item("AStartH") - 1) * 60
                                Else
                                    wTimeOff = (17 - DBDataSet1.Tables("Vacation").Rows(0).Item("AStartH")) * 60
                                End If
                            Else
                                If DBDataSet1.Tables("Vacation").Rows(0).Item("AEndH") > 12 Then
                                    wTimeOff = (DBDataSet1.Tables("Vacation").Rows(0).Item("AEndH") - 8 - 1) * 60
                                Else
                                    wTimeOff = (DBDataSet1.Tables("Vacation").Rows(0).Item("AEndH") - 8) * 60
                                End If
                            End If
                        End If
                    End If
            ...
            SumWTBase = SumWTBase + wBase
            SumWTOverTime = SumWTOverTime + wOverTime
            SumWTTimeOff = SumWTTimeOff + wTimeOff
            SumWTKousu1 = SumWTKousu1 + wBase + wOverTime + wTimeOff
            SumWTKousu2 = SumWTKousu2 + (wBase + wOverTime + wTimeOff) / 480

            e.Row.Cells(17).Text = FormatNumber(wBase, 0)
            e.Row.Cells(18).Text = FormatNumber(wOverTime, 0)
            e.Row.Cells(19).Text = FormatNumber(wTimeOff, 0)
            e.Row.Cells(20).Text = FormatNumber(wBase + wOverTime + wTimeOff, 0)
            e.Row.Cells(21).Text = FormatNumber((wBase + wOverTime + wTimeOff) / 480, 2)
        ...
        If e.Row.RowType = DataControlRowType.Footer Then
            ...
            '++  工時處理
            '++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            e.Row.Cells(17).Text = FormatNumber(SumWTBase, 0)
            e.Row.Cells(18).Text = FormatNumber(SumWTOverTime, 0)
            e.Row.Cells(19).Text = FormatNumber(SumWTTimeOff, 0)
            e.Row.Cells(20).Text = FormatNumber(SumWTKousu1, 0)
            e.Row.Cells(21).Text = FormatNumber(SumWTKousu2, 2)
