...
Partial Class WeekReport
    ...
    Dim wWeeks As Integer      '週別
    Dim wStartYear, wEndYear As String       '起迄年
    Dim wStartWeek, wEndWeek As Integer      '起迄週別
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "WeekReport.aspx"
        ...
        If Request.QueryString("pWeeks") <> "" And Request.QueryString("pReportYear") <> "" Then
            wNowYear = Request.QueryString("pReportYear")
            If CInt(Request.QueryString("pWeeks")) < 0 Then
                wWeeks = 1
            Else
                wWeeks = CInt(Request.QueryString("pWeeks"))
            End If
        Else
            SQL = "Select ReportYear, Weeks From V_WeekReport_01 Where ReportDate = '" + NowDate + "' "
            ...
            If DBTable1.Rows.Count > 0 Then
                ...
                wWeeks = DBTable1.Rows(0).Item("Weeks")
            Else
        ...
        wStartWeek = wWeeks   '起始週
        wEndYear = wStartYear       '截止年
        wEndWeek = wStartWeek       '截止週
        '
        '
        BNewReportBatch.Attributes("onclick") = "var ok=window.confirm('" + "是否開啟多筆登錄？" + "');if(!ok){return false;} else {CheckAttribute('NEWREPORT')}"
        '
        DFileName.Text = DUserID.Text + "_NewReport.xlsm"
        ...
        'SQL = "Select ReportYear, Weeks From V_WeekReport_01 "
        'SQL = SQL + "Where ReportYear = '" + wNowYear + "' "
        'SQL = SQL + "  And Weeks     > '" + CStr(wWeeks) + "' "
        'SQL = SQL + "Order by ReportYear, Weeks "
        'OleDbConnection1.Open()
        'Dim DBAdapter2 As New OleDbDataAdapter(SQL, OleDbConnection1)
        'DBAdapter2.Fill(DBDataSet1, "WeekReport")
        'DBTable1 = DBDataSet1.Tables("WeekReport")
        'If DBTable1.Rows.Count > 0 Then
        '    wEndYear = DBTable1.Rows(0).Item("ReportYear")  '截止年
        '    wEndWeek = DBTable1.Rows(0).Item("Weeks")      '截止週
        'Else
        '    wEndYear = wStartYear       '截止年
        '    wEndWeek = wStartWeek       '截止週
        'End If
    ...
    Sub SetFunHyperLink()
        ...
        'IT
        DFunLink1.Text = "IT"
        DFunLink1.NavigateUrl = "WeekReport.aspx?pUserID=" & wUserID & _
                                                         "&pReportYear=" & wNowYear & _
                                                         "&pWeeks=" & CStr(wWeeks) & _
                                                         "&pKeyField=" & "HeadDivision" & _
                                                         "&pKeyData=" & Server.UrlEncode("IT")
        DFunLink1.Visible = True
        '部門別/業務員別/客戶別
        If wKeyField <> "HeadDivision" Then
            DFunLink2.Text = wKeyData
            DFunLink2.NavigateUrl = "WeekReport.aspx?pUserID=" & wUserID & _
                                                             "&pReportYear=" & wNowYear & _
                                                             "&pWeeks=" & CStr(wWeeks) & _
        ...
        DWebConfig.Visible = True
        DWebConfig.NavigateUrl = uCommon.GetAppSetting("Http") + uCommon.GetAppSetting("UploadFilePath") + "WebConfig.xlsx"
    End Sub
    '---------------------------------------------------------------------------------------------------
    '*****************************************************************
    '**
    ...
    Sub ShowSchedule()
        ...
        SQL = SQL + "  And Weeks     >= '" + CStr(wStartWeek) + "' "
        SQL = SQL + "  And Weeks     <= '" + CStr(wEndWeek) + "' "
        SQL = SQL + "  And ( "
    ...
    '**
    '*****************************************************************
    Protected Sub BNewReport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BNewReport.Click
        Dim Cmd As String
        Dim URL As String
        URL = "NewReport.aspx?pUserID=" & wUserID & _
                                 "&pFun=" & "ADD"
        'Cmd = "<script>" + _
        '            "window.open('" & URL & "','新報告','status=1,toolbar=0,width=700,height=600,resizable=yes,scrollbars=yes');" + _
        '      "</script>"
        Cmd = "<script>" + _
                "window.open('" & URL & "');" + _
              "</script>"
    ...
    '**
    '*****************************************************************
    Protected Sub BNewReportBatch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BNewReportBatch.Click
    End Sub
    '---------------------------------------------------------------------------------------------------
    '*****************************************************************
    '**
    ...
    '**
    '*****************************************************************
    Protected Sub BMonth_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BMonth.Click
        Dim Cmd As String
        Dim URL As String
