...
<asp:DataList id=lstDiscussions runat="server" Summary="Discussion Design Table" CellPadding="4" DataKeyField="Parent" ItemStyle-Cssclass="Normal">
	<ItemTemplate>
		...
		<asp:hyperlink Text='<%# DataBinder.Eval(Container.DataItem, "Title") %>' NavigateUrl='' runat="server" />&nbsp;&nbsp;&nbsp;&nbsp;(<%# FormatUser(DataBinder.Eval(Container.DataItem,"CreatedByUser"),DataBinder.Eval(Container.DataItem,"CreatedDate", "{0:g}")) %>) 
  </ItemTemplate>
	<SelectedItemTemplate>
		...
		<asp:DataList id="DetailList" ItemStyle-Cssclass="Normal" datasource="<%# GetThreadMessages() %>" runat="server">
			<ItemTemplate>
				...
				<asp:hyperlink Text='<%# DataBinder.Eval(Container.DataItem, "Title") %>' NavigateUrl='<%# EditUrl("ItemID",DataBinder.Eval(Container.DataItem, "ItemID"),"Edit","itemindex=" & lstDiscussions.Items.Count) %>' runat="server" />
			</ItemTemplate>
		</asp:DataList>
	</SelectedItemTemplate>
</asp:DataList>
