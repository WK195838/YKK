...
#Region " Web Form 設計工具產生的程式碼 "
    ...
    Protected WithEvents DPDF As System.Web.UI.HtmlControls.HtmlInputFile
...
#End Region
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "MapSheet_01.aspx"
    ...
    Sub ShowFormData()
        ...
        If DBDataSet1.Tables("F_MapSheet").Rows.Count > 0 Then
            ...
            If DBDataSet1.Tables("F_MapSheet").Rows(0).Item("RefMapFile") <> "" Then
                LRefMapFile.NavigateUrl = Path & DBDataSet1.Tables("F_MapSheet").Rows(0).Item("RefMapFile")   '草圖
            Else
    ...
    Sub ShowSheetFunction()
        ...
        If wFormSno > 0 And wStep > 2 Then      '判斷是否[簽核]
            ...
            If DBDataSet1.Tables("T_WaitHandle").Rows.Count > 0 Then
                ...
                End If
                '連結顯示---需再修改
                LRefMapFile.Visible = True     '草圖
                LMapFile.Visible = True        '圖檔
                LPdfFile.Visible = True
        ...
        Else
            ...
            DReasonDesc.Visible = False     '延遲其他說明
            '連結隱藏
            LRefMapFile.Visible = False     '草圖
            LMapFile.Visible = False        '圖檔
            LPdfFile.Visible = False        '圖檔

            LBefOP.Visible = False          '工程履歷

            'OLD對應-Start 07-12-26
            'Top = 688
            'DMapSheet3.Visible = True  '圖面Sheet
            'DMapNo.Visible = True          '圖號
            'DMapFile.Visible = True        '圖檔
            'OLD-End
    ...
    Sub SetFieldAttribute(ByVal pPost As String)
        ...
        Select Case FindFieldInf("MapReqDelDate")
            ...
            Case Else   '隱藏
                ...
                BMapReqDelDate.Visible = True
        End Select
        If pPost = "New" Then DMapReqDelDate.Text = ""
        '光造型
    ...
    Sub AppendData(ByVal pFun As String, ByVal NewFormSno As Integer)
        ...
        SQl = SQl + "MapNo, MapFile,PdfFile, UPDSts, ModMap, ManufType, Suppiler, CreateUser, CreateTime, ModifyUser, "              '26~30
    ...
    Sub ModifyData(ByVal pFun As String, ByVal pSts As String)
        ...
        If DPdfFile.Visible Then
            If DPdfFile.PostedFile.FileName <> "" Then  '判斷有檔案上傳
                ...
                FileName = CStr(wFormSno) & "-" & "Pdf" & "-" & UploadDateTime & "-" & IO.Path.GetFileName(DPdfFile.PostedFile.FileName)
                '*** IE8對應-End
                Try    '上傳圖檔
                    DPdfFile.PostedFile.SaveAs(Path + FileName)
                Catch ex As Exception
