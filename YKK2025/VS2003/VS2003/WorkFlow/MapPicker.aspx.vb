...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        If Not Me.IsPostBack Then   '不是PostBack
            If (Request.QueryString("field") = "Ori") Or (Request.QueryString("field") = "Mod") Then
                ...
                End If
            Else
                If Request.Cookies("MapNo1").Value <> "" Then
                    DKey.Text = Request.Cookies("MapNo1").Value
                    pKey = Request.Cookies("MapNo1").Value
                Else
                    DKey.Text = ""
                    pKey = "ALL"
                End If
    ...
    Sub MapData()
        ...
        If (Request.QueryString("field") = "Ori") Or (Request.QueryString("field") = "Ori1") Then
            SQL = "SELECT "   '取得DB資料
    ...
    Private Sub DataGrid1_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles DataGrid1.ItemCommand
        If e.CommandSource.CommandName = "Select" Then  '點選選取檢查
            ...
            Else
                Response.Cookies("MapNo1").Value = Key
            End If
            ...
            If (Request.QueryString("field") = "Ori") Or (Request.QueryString("field") = "Ori1") Then
                SQL = "SELECT "   '取得DB資料
                SQL = SQL & "FormNo, FormSno, Level, Cpsc "
                SQL = SQL & "FROM F_MapSheet "
            Else
                SQL = "SELECT "   '取得DB資料
                SQL = SQL & "FormNo, FormSno, Level, Cpsc "
            ...
            If (Request.QueryString("field") = "Ori") Or (Request.QueryString("field") = "Mod") Then
                Response.Write(String.Format("<script>window.opener.document.{0}.value = '{1:d}'; window.opener.document.{2}.value = '{3:d}'; window.opener.document.{4}.value = '{5:d}'; window.opener.document.{6}.value = '{7:d}'; window.opener.document.{8}.value = '{9:d}'; window.close();</script>", "Form1.DMapNo", Key, "Form1.DLevel", wLevel, "Form1.DOFormNo", wFormNo, "Form1.DOFormSno", wFormSno, "Form1.DCpsc", wCpsc))
            Else
                Response.Write(String.Format("<script>window.opener.document.{0}.value = '{1:d}'; window.opener.document.{2}.value = '{3:d}'; window.opener.document.{4}.value = '{5:d}'; window.opener.document.{6}.value = '{7:d}'; window.opener.document.{8}.value = '{9:d}'; window.close();</script>", "Form1.DMapNo1", Key, "Form1.DLevel", wLevel, "Form1.DOFormNo1", wFormNo, "Form1.DOFormSno1", wFormSno, "Form1.DCpsc", wCpsc))
            End If
