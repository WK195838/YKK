...
#End Region
    ...
    Sub DataList()
        ...
        If wFun = "CT" Then
            ...
            SQL = SQL + "'ManufOutCTSheet_02.aspx?' + "
            SQL = SQL + "'pFormNo='   + FormNo + "
            SQL = SQL + "'&pFormSno=' + str(FormSno,Len(FormSno)) "
            SQL = SQL + " As URL "
            SQL = SQL + "FROM F_ManufOutCTSheet "
            SQL = SQL + "Where OFormNo = '" & wFormNo & "'"
            SQL = SQL + "  and OFormSno = '" & CStr(wFormSno) & "'"
        End If
        If wFun = "SD" Then
            SQL = "SELECT "
            SQL = SQL + "Case Sts When 0 then '未結' When 1 then '已結OK' When 2 then '已結NG' else '抽單' end as StsDesc, "
            SQL = SQL + "No, Convert(Varchar, Date,111) as DateDesc, '' as SliderCode, '' as MapNo, "
            SQL = SQL + "OFormNo+'-'+str(OFormSno,Len(OFormSno)) as OFormDesc, "
            SQL = SQL + "'' as NFormDesc, "
            SQL = SQL + "'ManufOutSDSheet_02.aspx?' + "
            SQL = SQL + "'pFormNo='   + FormNo + "
            SQL = SQL + "'&pFormSno=' + str(FormSno,Len(FormSno)) "
            SQL = SQL + " As URL "
            SQL = SQL + "FROM F_ManufOutSDSheet "
            SQL = SQL + "Where SliderGRCode = '" & wCode & "'"
        End If
        If wFun = "SF" Then
            SQL = "SELECT "
            SQL = SQL + "Case Sts When 0 then '未結' When 1 then '已結OK' When 2 then '已結NG' else '抽單' end as StsDesc, "
