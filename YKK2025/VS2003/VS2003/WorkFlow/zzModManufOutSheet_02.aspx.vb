...
Public Class ModManufOutSheet_02
...
#End Region
    ...
    Sub ShowSheetField(ByVal pPost As String)
        ...
        oFieldAttribute.pFormNo = "900004"     '表單號碼
    ...
    '**
    '*****************************************************************
    Private Sub DMaterial_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'DB連結設定
        Dim OleDbConnection1 As New OleDbConnection
        OleDbConnection1.ConnectionString = ConfigurationSettings.AppSettings.Get("SqlConn")  'SQL連結設定
        OleDbConnection1.Open()
    ...
    Private Sub BSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BSave.Click
        ...
        If ErrCode = 0 Then
            ...
            If wFormSno > 0 Then    '判斷是否有資料
                ...
                RtnCode = oPutSubFile.PutSlider(DSliderCode.Text, DSliderGRCode.Text, "900004", wFormSno, Request.Cookies("UserID").Value)
                'Sample Table處理
                RtnCode = oPutSubFile.PutSample(DSample.Text, "900004", wFormSno, Request.Cookies("UserID").Value)
                'Price Table處理
                RtnCode = oPutSubFile.PutPrice(DPrice.Text, "900004", wFormSno, Request.Cookies("UserID").Value)
                'QA1 Table處理
                RtnCode = oPutSubFile.PutQA1(DQuality1.Text, "900004", wFormSno, Request.Cookies("UserID").Value)
                'QA2 Table處理
                RtnCode = oPutSubFile.PutQA2(DQuality2.Text, "900004", wFormSno, Request.Cookies("UserID").Value)
            Else
                '取得表單流水號
                ...
                Else
                    ...
                    'Slider Table處理
                    oPutSubFile = Server.CreateObject("MSubFile.SubFile")
                    RtnCode = oPutSubFile.PutSlider(DSliderCode.Text, DSliderGRCode.Text, "900004", NewFormSno, Request.Cookies("UserID").Value)
                    'Sample Table處理
                    RtnCode = oPutSubFile.PutSample(DSample.Text, "900004", NewFormSno, Request.Cookies("UserID").Value)
                    'Price Table處理
                    RtnCode = oPutSubFile.PutPrice(DPrice.Text, "900004", NewFormSno, Request.Cookies("UserID").Value)
                    'QA1 Table處理
                    RtnCode = oPutSubFile.PutQA1(DQuality1.Text, "900004", NewFormSno, Request.Cookies("UserID").Value)
                    'QA2 Table處理
                    RtnCode = oPutSubFile.PutQA2(DQuality2.Text, "900004", NewFormSno, Request.Cookies("UserID").Value)
                End If
            End If
        Else
