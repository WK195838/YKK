...
Public Class IT_ChangeDataSheet_01
...
#Region " Web Form 設計工具產生的程式碼 "
    ...
    Protected WithEvents DChangeDataSheet1 As System.Web.UI.WebControls.Image
...
#End Region
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "IT_ChangeDataSheet_01.aspx"
    ...
    '*****************************************************************
    Sub ShowFormData()
        Dim Path As String = ConfigurationSettings.AppSettings.Get("Http") & ConfigurationSettings.AppSettings.Get("ChangeDataFilePath")
        ...
        SQL = "Select * From F_ChangeDataSheet "
        SQL = SQL & " Where FormNo =  '" & wFormNo & "'"
        SQL = SQL & "   And FormSno =  '" & CStr(wFormSno) & "'"
        Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
        DBAdapter1.Fill(DBDataSet1, "F_ChangeDataSheet")
        If DBDataSet1.Tables("F_ChangeDataSheet").Rows.Count > 0 Then
            '表單資料
            DNo.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("No")
            DDate.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("Date")
            DName.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("Name")
            DEmpID.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("EmpID")
            DJobTitle.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("JobTitle")
            DJobCode.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("JobCode")
            DDepoName.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("DepoName")
            DDepoCode.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("DepoCode")
            DDivision.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("Division")
            DDivisionCode.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("DivisionCode")
            DFinishDate.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("FinishDate")
            DTarget.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("Target")

            If DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("RefFile") <> "" Then
                LRefFile.NavigateUrl = Path & DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("RefFile")
            Else
                LRefFile.Visible = False
            End If
            If DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("BStartDate") = "1900/1/1" Then
                DBStartDate.Text = ""
            Else
                DBStartDate.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("BStartDate")
            End If

            If DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("BEndDate") = "1900/1/1" Then
                DBEndDate.Text = ""
            Else
                DBEndDate.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("BEndDate")
            End If

            DBDays.Text = CStr(DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("BDays"))
            SetFieldData("Engineer", DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("Engineer"))
            SetFieldData("System", DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("System"))
            DRemark.Text = DBDataSet1.Tables("F_ChangeDataSheet").Rows(0).Item("Remark")
            '交易資料
            DBDataSet1.Clear()
    ...
    Sub ShowSheetFunction()
        ...
        If wFormSno > 0 And wStep > 2 Then    '判斷是否[簽核]
            ...
            If DBDataSet1.Tables("T_WaitHandle").Rows.Count > 0 Then
                'Sheet顯示
                DChangeDataSheet1.Visible = True   '表單Sheet-1
    ...
    Sub SetFieldData(ByVal pFieldName As String, ByVal pName As String)
        ...
        If pFieldName = "Engineer" Then
            ...
            If idx = 0 Then
                ...
                End If
            Else
                SQL = "Select * From M_Referp Where Cat='1103' and DKey='ENGINEER' Order by Data "
                Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
        ...
        If pFieldName = "System" Then
            ...
            If idx = 0 Then
                ...
                End If
            Else
                SQL = "Select * From M_Referp Where Cat='1103' and DKey='SYSTEM' Order by Data "
                Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
                DBAdapter1.Fill(DBDataSet1, "M_Referp")
                DBTable1 = DBDataSet1.Tables("M_Referp")
                For i = 0 To DBTable1.Rows.Count - 1
    ...
    Sub FlowControl(ByVal pFun As String, ByVal pAction As Integer, ByVal pSts As String)
        ...
        If ErrCode = 0 Then
            If DNo.Text <> "" Then
                ...
                ErrCode = oCheckNo.CommissionNo("001103", wFormSno, wStep, DNo.Text)    '表單號碼, 表單流水號, 工程, 委託書No
                If ErrCode <> 0 Then
                    ErrCode = 9060
                End If
            End If
    ...
    Sub AppendData(ByVal pFun As String, ByVal NewFormSno As Integer)
        Dim Path As String = Server.MapPath(ConfigurationSettings.AppSettings.Get("ChangeDataFilePath"))
        Dim UploadDateTime = CStr(DateTime.Now.Year) + CStr(DateTime.Now.Month) + CStr(DateTime.Now.Day) + _
                             CStr(DateTime.Now.Hour) + CStr(DateTime.Now.Minute) + CStr(DateTime.Now.Second)     '上傳日期
        ...
        SQl = "Insert into F_ChangeDataSheet "
        SQl = SQl + "( "
    ...
    Sub ModifyData(ByVal pFun As String, ByVal pSts As String)
        ...
        SQl = "Update F_ChangeDataSheet Set "
        If pFun <> "SAVE" Then
            SQl = SQl + " Sts = '" & pSts & "',"
