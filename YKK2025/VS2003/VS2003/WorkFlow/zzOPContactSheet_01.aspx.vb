...
Public Class OPContactSheet_01
...
#End Region
    ...
    '*****************************************************************
    Sub ShowFormData()
        Dim Path As String = ConfigurationSettings.AppSettings.Get("Http") & ConfigurationSettings.AppSettings.Get("OPContactFilePath")
        ...
        SQL = "Select * From F_OPContactSheet "
        SQL = SQL & " Where Sts = 0 "
        SQL = SQL & "   And FormNo =  '" & wFormNo & "'"
        SQL = SQL & "   And FormSno =  '" & CStr(wFormSno) & "'"
        Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
        DBAdapter1.Fill(DBDataSet1, "F_OPContactSheet")
        If DBDataSet1.Tables("F_OPContactSheet").Rows.Count > 0 Then
            DNo.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("No")                       'No
            DDate.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("Date")                   '日期
            SetFieldData("Division", DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("Division"))  '部門
            SetFieldData("Person", DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("Person"))      '擔當
            DSliderCode.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("SliderCode")       'Slider Code
            SetFieldData("Level", DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("Level"))                '難易度
            DMapNo.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("MapNo")             '圖號
            If DMapNo.Text <> "" Then
            ...
            DOFormNo.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("OFormNo")             '圖號
            DOFormSno.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("OFormSno")             '圖號
            ...
            DNFormNo.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("NFormNo")             '圖號
            If DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("NFormSno") = 0 Then
                DNFormSno.Text = ""
            Else
                DNFormSno.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("NFormSno")             '圖號
            End If

            If DNFormNo.Text <> "" And DNFormSno.Text <> "" Then
                LNFormNo.NavigateUrl = "ModManufSheet_01.aspx?pFormNo=" & DNFormNo.Text & "&pFormSno=" & CInt(DNFormSno.Text) & "&pOFormNo=" & DOFormNo.Text & "&pOFormSno=" & CInt(DOFormSno.Text) & "&pStep=" & wStep
            ...
            DTarget.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("Target")             '圖號
            DContent.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("Content")             '圖號
            DDReason.Text = DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("Reason")             '圖號
            If DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("AttachFile") <> "" Then          '圖檔1
                LAttachFile.NavigateUrl = Path & DBDataSet1.Tables("F_OPContactSheet").Rows(0).Item("AttachFile")
            Else
    ...
    Private Sub BSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BSave.Click
        ...
        '儲存資料
        If ErrCode = 0 Then
            Dim Path As String = Server.MapPath(ConfigurationSettings.AppSettings.Get("OPContactFilePath"))
            ...
            '儲存表單資料
            SQL = "Update F_OPContactSheet Set "
            SQL = SQL + " No = '" & DNo.Text & "',"
                ...
                If ErrCode = 0 Then
                    ...
                    If wFormSno = 0 And wStep = 1 Then    '判斷是否起單
                        If NewFormSno <> 0 Then
                            '表單Table處理
                            SQL = "Insert into F_OPContactSheet "
                            SQL = SQL + "( "
