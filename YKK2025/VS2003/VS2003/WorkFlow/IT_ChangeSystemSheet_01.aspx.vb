...
Public Class IT_ChangeSystemSheet_01
...
#Region " Web Form 設計工具產生的程式碼 "
    ...
    Protected WithEvents DChangeSystemSheet1 As System.Web.UI.WebControls.Image
...
#End Region
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "IT_ChangeSystemSheet_01.aspx"
    ...
    '*****************************************************************
    Sub ShowFormData()
        Dim Path As String = ConfigurationSettings.AppSettings.Get("Http") & ConfigurationSettings.AppSettings.Get("ChangeSystemFilePath")
        ...
        SQL = "Select * From F_ChangeSystemSheet "
        SQL = SQL & " Where FormNo =  '" & wFormNo & "'"
        SQL = SQL & "   And FormSno =  '" & CStr(wFormSno) & "'"
        Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
        DBAdapter1.Fill(DBDataSet1, "F_ChangeSystemSheet")
        If DBDataSet1.Tables("F_ChangeSystemSheet").Rows.Count > 0 Then
            '表單資料
            DNo.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("No")
            DDate.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("Date")
            DName.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("Name")
            DEmpID.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("EmpID")
            DJobTitle.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("JobTitle")
            DJobCode.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("JobCode")
            DDepoName.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("DepoName")
            DDepoCode.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("DepoCode")
            DDivision.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("Division")
            DDivisionCode.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("DivisionCode")
            DFinishDate.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("FinishDate")
            DTarget.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("Target")
            DEffect.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("Effect")

            If DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("RefFile") <> "" Then
                LRefFile.NavigateUrl = Path & DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("RefFile")
            Else
                LRefFile.Visible = False
            End If
            If DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("BStartDate") = "1900/1/1" Then
                DBStartDate.Text = ""
            Else
                DBStartDate.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("BStartDate")
            End If

            If DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("BEndDate") = "1900/1/1" Then
                DBEndDate.Text = ""
            Else
                DBEndDate.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("BEndDate")
            End If

            DBDays.Text = CStr(DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("BDays"))
            SetFieldData("Engineer", DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("Engineer"))
            SetFieldData("DevelopItem", DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("DevelopItem"))
            SetFieldData("System", DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("System"))
            DITEffect.Text = DBDataSet1.Tables("F_ChangeSystemSheet").Rows(0).Item("ITEffect")
            '交易資料
            DBDataSet1.Clear()
    ...
    Sub ShowSheetFunction()
        ...
        If wFormSno > 0 And wStep > 2 Then    '判斷是否[簽核]
            ...
            If DBDataSet1.Tables("T_WaitHandle").Rows.Count > 0 Then
                'Sheet顯示
                DChangeSystemSheet1.Visible = True   '表單Sheet-1
    ...
    Sub SetFieldAttribute(ByVal pPost As String)
        ...
        '效果
        Select Case FindFieldInf("Effect")
            Case 0  '顯示
                DEffect.BackColor = Color.LightGray
                DEffect.ReadOnly = True
                DEffect.Visible = True
            Case 1  '修改+檢查
                DEffect.BackColor = Color.GreenYellow
                ShowRequiredFieldValidator("DEffectRqd", "DEffect", "異常：需輸入效果")
                DEffect.Visible = True
            Case 2  '修改
                DEffect.BackColor = Color.Yellow
                DEffect.Visible = True
            Case Else   '隱藏
                DEffect.Visible = False
        End Select
        If pPost = "New" Then DEffect.Text = ""
        '參考附件
        ...
        '列入開發
        Select Case FindFieldInf("DevelopItem")
            Case 0  '顯示
                DDevelopItem.BackColor = Color.LightGray
                DDevelopItem.Visible = True
            Case 1  '修改+檢查
                DDevelopItem.BackColor = Color.GreenYellow
                ShowRequiredFieldValidator("DDevelopItemRqd", "DDevelopItem", "異常：需輸入列入開發")
                DDevelopItem.Visible = True
            Case 2  '修改
                DDevelopItem.BackColor = Color.Yellow
                DDevelopItem.Visible = True
            Case Else   '隱藏
                DDevelopItem.Visible = False
        End Select
        If pPost = "New" Then SetFieldData("DevelopItem", "ZZZZZZ")
        'IT效果
        Select Case FindFieldInf("ITEffect")
            Case 0  '顯示
                DITEffect.BackColor = Color.LightGray
                DITEffect.ReadOnly = True
                DITEffect.Visible = True
            Case 1  '修改+檢查
                DITEffect.BackColor = Color.GreenYellow
                ShowRequiredFieldValidator("DITEffectRqd", "DITEffect", "異常：需輸入資訊部門效果")
                DITEffect.Visible = True
            Case 2  '修改
                DITEffect.BackColor = Color.Yellow
                DITEffect.Visible = True
            Case Else   '隱藏
                DITEffect.Visible = False
        End Select
        If pPost = "New" Then DITEffect.Text = ""
    End Sub
    ...
    Sub SetFieldData(ByVal pFieldName As String, ByVal pName As String)
        ...
        If pFieldName = "Engineer" Then
            ...
            If idx = 0 Then
                ...
                End If
            Else
                SQL = "Select * From M_Referp Where Cat='1102' and DKey='ENGINEER' Order by Data "
                Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
        ...
        End If
        '列入開發
        If pFieldName = "DevelopItem" Then
            ...
            If idx = 0 Then
                If pName <> "ZZZZZZ" Then
                    ...
                    ListItem1.Text = pName
                    ListItem1.Value = pName
                    DDevelopItem.Items.Add(ListItem1)
                End If
            Else
                SQL = "Select * From M_Referp Where Cat='1102' and DKey='DEVELOPITEM' Order by Data "
                Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
                DBAdapter1.Fill(DBDataSet1, "M_Referp")
                DBTable1 = DBDataSet1.Tables("M_Referp")
                For i = 0 To DBTable1.Rows.Count - 1
        ...
        If pFieldName = "System" Then
            ...
            If idx = 0 Then
                ...
                End If
            Else
                SQL = "Select * From M_Referp Where Cat='1102' and DKey='SYSTEM' Order by Data "
                Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
                DBAdapter1.Fill(DBDataSet1, "M_Referp")
                DBTable1 = DBDataSet1.Tables("M_Referp")
                For i = 0 To DBTable1.Rows.Count - 1
    ...
    Sub FlowControl(ByVal pFun As String, ByVal pAction As Integer, ByVal pSts As String)
        ...
        If ErrCode = 0 Then
            If DNo.Text <> "" Then
                ...
                ErrCode = oCheckNo.CommissionNo("001102", wFormSno, wStep, DNo.Text)    '表單號碼, 表單流水號, 工程, 委託書No
                If ErrCode <> 0 Then
                    ErrCode = 9060
                End If
            End If
    ...
    Sub AppendData(ByVal pFun As String, ByVal NewFormSno As Integer)
        Dim Path As String = Server.MapPath(ConfigurationSettings.AppSettings.Get("ChangeSystemFilePath"))
        Dim UploadDateTime = CStr(DateTime.Now.Year) + CStr(DateTime.Now.Month) + CStr(DateTime.Now.Day) + _
                             CStr(DateTime.Now.Hour) + CStr(DateTime.Now.Minute) + CStr(DateTime.Now.Second)     '上傳日期
        ...
        SQl = "Insert into F_ChangeSystemSheet "
        SQl = SQl + "( "
        ...
        SQl = SQl + "BDays, FinishDate, Target, Effect, RefFile, "
        SQl = SQl + "BStartDate, BEndDate, Engineer, DevelopItem, ITEffect, System, "
        ...
        SQl = SQl + " '" + DDevelopItem.SelectedValue + "', "
        SQl = SQl + " N'" + DITEffect.Text + "', "
    ...
    Sub ModifyData(ByVal pFun As String, ByVal pSts As String)
        ...
        SQl = "Update F_ChangeSystemSheet Set "
        If pFun <> "SAVE" Then
            SQl = SQl + " Sts = '" & pSts & "',"
        ...
        SQl = SQl + " DevelopItem = '" & DDevelopItem.SelectedItem.Text & "',"
        SQl = SQl + " ITEffect = N'" & DITEffect.Text & "',"
