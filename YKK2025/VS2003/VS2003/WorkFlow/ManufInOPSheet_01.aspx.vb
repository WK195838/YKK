...
Public Class ManufInOPSheet_01
...
#End Region
    ...
    Sub SetParameter()
        ...
        Response.Cookies("PGM").Value = "ManufInOPSheet_01.aspx"
    ...
    '*****************************************************************
    Sub ShowFormData()
        Dim Path As String = ConfigurationSettings.AppSettings.Get("Http") & ConfigurationSettings.AppSettings.Get("ManufInOPFilePath")
        ...
        SQL = "Select * From F_ManufInOPSheet "
        SQL = SQL & " Where FormNo =  '" & wFormNo & "'"
        SQL = SQL & "   And FormSno =  '" & CStr(wFormSno) & "'"
        Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
        DBAdapter1.Fill(DBDataSet1, "F_ManufInOPSheet")
        If DBDataSet1.Tables("F_ManufInOPSheet").Rows.Count > 0 Then
            DNo.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("No")                       'No
            DDate.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("Date")                   '日期
            SetFieldData("Division", DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("Division"))  '部門
            SetFieldData("Person", DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("Person"))      '擔當
            DSliderCode.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("SliderCode")       'Slider Code
            DLevel.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("Level")                 '難易度
            DMapNo.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("MapNo")                 '圖號
            If DMapNo.Text <> "" Then
            ...
            DOFormNo.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("OFormNo")           '表單
            DOFormSno.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("OFormSno")         '單號
            DSuppiler.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("Suppiler")         '外注商
            DMakeCAM.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("MakeCAM")         'make cam
            DQCLT.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("QCLT")               'QC-L/T
            ...
            DNFormNo.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("NFormNo")             '圖號
            If DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("NFormSno") = 0 Then
                DNFormSno.Text = ""
            Else
                DNFormSno.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("NFormSno")       '圖號
            End If
            ...
            DTarget.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("Target")             '圖號
            DContent.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("Content")             '圖號
            DDReason.Text = DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("Reason")             '圖號
            If DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("AttachFile") <> "" Then          '圖檔1
                LAttachFile.NavigateUrl = Path & DBDataSet1.Tables("F_ManufInOPSheet").Rows(0).Item("AttachFile")
            Else
    ...
    Sub SetPopupFunction()
        ...
        BIn.Attributes("onclick") = "DevNoPicker('In','000004');"       '內製
        BOMapNo.Attributes("onclick") = "MapPicker('Ori','000004');"    '原始圖號
        BMMapNo.Attributes("onclick") = "MapPicker('Mod','000004');"    '修改圖號
    ...
    Sub SetFieldAttribute(ByVal pPost As String)
        ...
        End Select
        If pPost = "New" Then DMakeCAM.Text = ""
    ...
    Sub FlowControl(ByVal pFun As String, ByVal pAction As Integer, ByVal pSts As String)
        ...
        If ErrCode = 0 Then
            If DNo.Text <> "" Then
                ...
                ErrCode = oCheckNo.CommissionNo("000004", wFormSno, wStep, DNo.Text)    '表單號碼, 表單流水號, 工程, 委託書No
                If ErrCode <> 0 Then
                    ErrCode = 9070
                End If
            End If
        ...
        If ErrCode = 0 Then
            ...
            While Run = True
                ...
                If ErrCode = 0 And pRunNextStep = 1 Then
                    ...
                    SQL = SQL & " Where Active = 1 "
                    SQL = SQL & "   And UserName =  '" & DMakeCAM.Text & "'"
                    Dim DBAdapter1 As New OleDbDataAdapter(SQL, OleDbConnection1)
                    DBAdapter1.Fill(DBDataSet1, "M_Users")
    ...
    Sub AppendData(ByVal pFun As String, ByVal NewFormSno As Integer)
        Dim Path As String = Server.MapPath(ConfigurationSettings.AppSettings.Get("ManufInOPFilePath"))
        ...
        SQl = "Insert into F_ManufInOPSheet "
        SQl = SQl + "( "
        SQl = SQl + "Sts, CompletedTime, FormNo, FormSNo, "          '1~4
        SQl = SQl + "No, Date, Division, Person, SliderCode, "       '5~9
        SQl = SQl + "Level, MapNo, OFormNo, OFormSno, NFormNo, NFormSno, Suppiler, MakeCAM,  QCLT, "  '10~14
        ...
        SQl = SQl + " N'" + YKK.ReplaceString(DMakeCAM.Text) + "', "           'make cam
    ...
    Sub ModifyData(ByVal pFun As String, ByVal pSts As String)
        ...
        SQl = "Update F_ManufInOPSheet Set "
        If pFun <> "SAVE" Then
            SQl = SQl + " Sts = '" & pSts & "',"
        ...
        SQl = SQl + " MakeCAM = N'" & YKK.ReplaceString(DMakeCAM.Text) & "',"
    ...
    Sub ReplaceManufInData()
        ...
        If DBDataSet1.Tables("F_ManufInModSheet").Rows.Count > 0 Then
            ...
            SQl = SQl + " MakeCAM = N'" & DBDataSet1.Tables("F_ManufInModSheet").Rows(0).Item("MakeCAM") & "',"
            SQl = SQl + " QCLT = '" & DBDataSet1.Tables("F_ManufInModSheet").Rows(0).Item("QCLT") & "',"
